...
public class SendController extends TransferTransactionController  {
    ...
    private TransferTransactionV2 signTransaction() {
        ...
        return Transactions.makeTransferTx(
                ...
                asset.getAssetId(),
                feeTextField.getText().isEmpty() ? Tac.FEE : toLong(feeTextField.getText(), assetFee.getDecimals()),
                assetFee.getAssetId(),
    ...
    private void updateComboBoxes(List<Transferable> assetList) {
        ...
        final Predicate<Transferable> filter = t ->
                t.getMinFee() != null && t.balanceAsLong() >= t.minFeeAsLong() && t.sponsorBalanceAsLong() >= Tac.FEE;
        final var feeList = assetList.stream()
                .filter(filter)
