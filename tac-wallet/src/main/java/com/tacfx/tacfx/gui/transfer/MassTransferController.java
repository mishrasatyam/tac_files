...
import com.tacfx.tacfx.gui.assets.AssetCell;
import com.tacfx.tacfx.gui.assets.AssetConverterProfile;
import com.tacfx.tacfx.gui.dialog.ConfirmMassTransferController;
import com.tacfx.tacfx.gui.style.StyleHandler;
import com.tacfx.tacfx.logic.AddressValidator;
...
import static com.tacfx.tacfx.logic.FormValidator.MASS_TX_PATTERN;
...
public class MassTransferController extends TransferTransactionController  {
    ...
    private boolean isValidFee(final long fee) {
        ...
        final var tac = assets.get();
        final var tacBalance = toLong(tac.getBalance(), tac.getDecimals());

        if (selectedAsset.get().getAssetId().equals(mainToken.getAssetId())) {
            final var assetAmount = toLong(amountTextField.getText(), tac.getDecimals());
            return fee <= tacBalance - assetAmount - fee;
        }
        return fee <= tacBalance - fee;
    }
