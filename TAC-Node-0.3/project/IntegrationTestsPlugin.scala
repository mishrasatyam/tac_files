...
object IntegrationTestsPlugin extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    inConfig(Test)(
            ...
            Tests.SubProcess(
              ForkOptions(
                ...
                runJVMOptions = Vector(
                  "-Dtac.it.logging.appender=FILE",
                  s"-Dtac.it.logging.dir=${logDirectoryValue / suite.name.replaceAll("""(\w)\w*\.""", "$1.")}" // foo.bar.Baz -> f.b.Baz
                ) ++ javaOptionsValue,
                connectInput = false,
      ...
      Seq(
        maxParallelSuites := Option(Integer.getInteger("tac.it.max-parallel-suites"))
          .getOrElse[Integer] {
            try {
