...
object ExtensionPackaging extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      // To write files to Tac NODE directory
      ...
      javaOptions in run ++= extensionClasses.value.zipWithIndex.map { case (extension, index) => s"-Dtac.extensions.$index=$extension" }
  ...
  private def maintainerFix =
    inConfig(Linux)(
      Seq(
        ...
        packageSummary := s"Tac node ${name.value}${network.value.packageSuffix} extension",
        packageDescription := s"Tac node ${name.value}${network.value.packageSuffix} extension"
      ))
