...
import com.tacplatform.lang.v1.repl.node.BlockchainUnavailableException
...
class ReplTest extends PropSpec with ScriptGen with Matchers with NoShrink {
  ...
  property("tac context funcs absent") {
    val repl = Repl()
  ...
  property("url slash strip") {
    val url = "testnodes.tacnodes.com"
  ...
  property("reconfigure") {
    ...
    val settings = NodeConnectionSettings("testnodes.tacnodes.com", 'T'.toByte, address1)
  ...
  property("libraries") {
    ...
    val settings = NodeConnectionSettings("testnodes.tacnodes.com", 'T'.toByte, address)
    val repl = Repl(
      Some(settings),
      libraries = List(
      """
