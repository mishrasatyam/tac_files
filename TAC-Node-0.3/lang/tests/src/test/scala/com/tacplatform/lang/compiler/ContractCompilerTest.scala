...
import com.tacplatform.lang.Common.{NoShrink, produce}
...
import com.tacplatform.lang.v1.compiler.{CompilerContext, Terms}
import com.tacplatform.lang.v1.estimator.v3.ScriptEstimatorV3
import com.tacplatform.lang.v1.evaluator.FunctionIds
import com.tacplatform.lang.v1.evaluator.ctx.impl.tac.{FieldNames, Types, TacContext}
...
import com.tacplatform.protobuf.dapp.DAppMeta.CallableFuncSignature
...
class ContractCompilerTest extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
    ...
    Monoid
      .combineAll(
        Seq(
          ...
          CryptoContext.build(com.tacplatform.lang.Global, V3).withEnvironment[Environment],
          TacContext.build(
            Global,
    ...
    Monoid
      .combineAll(
        Seq(
          ...
          CryptoContext.build(com.tacplatform.lang.Global, V4).withEnvironment[Environment],
          TacContext.build(
            Global,
  ...
  property("contract compiles fails when incorrect return type") {
    import com.tacplatform.lang.v1.evaluator.ctx.impl._
    ...
    val expr = {
      val script =
        """
          ...
          |   else TransferSet([ScriptTransfer(i.caller, tacBalance(this), base58'somestr')])
          | }
          |
          | @Verifier(t)
          | func verify() = {
  ...
  property("tacBalanceV4 have type BalanceDetails") {
    val ctx = dAppV4Ctx
    val expr = {
      val script =
        """
          | @Callable(i)
          | func bar() = {
          |   [ScriptTransfer(i.caller, tacBalance(this), base58'somestr')]
          | }
        """.stripMargin
      Parser.parseContract(script).get.value
    }
  ...
  property("tacBalanceV4 have type BalanceDetails with fields") {
    val ctx = dAppV4Ctx
    val expr = {
      val script =
        """
          ...
          | func bar() = {
          |   [
          |     ScriptTransfer(i.caller, tacBalance(this).available, base58'somestr'),
          |     ScriptTransfer(i.caller, tacBalance(this).regular, base58'somestr'),
          |     ScriptTransfer(i.caller, tacBalance(this).generating, base58'somestr'),
          |     ScriptTransfer(i.caller, tacBalance(this).effective, base58'somestr')
          |   ]
          | }
        """.stripMargin
      Parser.parseContract(script).get.value
  ...
  property("assetBalanceV3 allow issued assets and tac") {
    val ctx = Monoid
      .combineAll(
        Seq(
          PureContext.build(V3).withEnvironment[Environment],
  ...
  property("@Callable V4 result type") {
    ...
    val ctx =
      PureContext.build(V4).withEnvironment[Environment] |+|
        TacContext.build(Global, DirectiveSet(V4, Account, DAppType).explicitGet())
  ...
  property("@Callable Invoke") {
    ...
    val ctx =
      PureContext.build(V5).withEnvironment[Environment] |+|
        TacContext.build(Global, DirectiveSet(V5, Account, DAppType).explicitGet())
