package com.tacplatform.api.grpc
...
import com.tacplatform.protobuf.utils.PBImplicitConversions.fromAssetIdAndAmount
...
class AccountsApiGrpcImpl(commonApi: CommonAccountsApi)(implicit sc: Scheduler) extends AccountsApiGrpc.AccountsApi {
  ...
  private def loadTacBalance(address: Address): BalanceResponse = {
    val details = commonApi.balanceDetails(address)
    BalanceResponse().withTac(
      BalanceResponse.TacBalances(
        details.regular,
        details.generating,
        details.available,
        details.effective,
  ...
  override def getBalances(request: BalancesRequest, responseObserver: StreamObserver[BalanceResponse]): Unit = responseObserver.interceptErrors {
    ...
    val assetIds: Seq[Asset]           = request.assets.map(id => if (id.isEmpty) Asset.Tac else Asset.IssuedAsset(id.toByteStr))

    val responseStream = (addressOption, assetIds) match {
      case (Some(address), Seq()) =>
        Observable(loadTacBalance(address)) ++ commonApi.portfolio(address).map(assetBalanceResponse)
      case (Some(address), nonEmptyList) =>
        Observable
          .fromIterable(nonEmptyList)
          .map {
            case Asset.Tac           => loadTacBalance(address)
