...
import com.tacplatform.api.common.CommonBlocksApi
...
import com.tacplatform.events.StateUpdate.{AssetInfo, AssetStateUpdate, BalanceUpdate, DataEntryUpdate, LeaseUpdate, LeasingBalanceUpdate}
import com.tacplatform.events.StateUpdate.LeaseUpdate.LeaseStatus
import com.tacplatform.events.api.grpc.BlockchainUpdatesApiGrpcImpl
import com.tacplatform.events.api.grpc.protobuf.{GetBlockUpdatesRangeRequest, SubscribeEvent, SubscribeRequest}
import com.tacplatform.events.protobuf.serde._
import com.tacplatform.events.repo.UpdatesRepoImpl
import com.tacplatform.features.BlockchainFeatures
import com.tacplatform.history.Domain
import com.tacplatform.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings, TacSettings}
import com.tacplatform.state.{AssetDescription, Blockchain, EmptyDataEntry, Height, LeaseBalance, StringDataEntry}
import com.tacplatform.state.diffs.BlockDiffer
import com.tacplatform.transaction.{PaymentTransaction, TxHelpers}
...
class BlockchainUpdatesSpec extends FreeSpec with Matchers with WithDomain with ScalaFutures with PathMockFactory {
  var currentSettings: TacSettings = domainSettingsWithFS(
  ...
  "BlockchainUpdates" - {
    ...
    "should include correct tac amount" in withNEmptyBlocksSubscription() { result =>
      val balances = result.collect { case b: BlockAppended => b.updatedTacAmount }
    ...
    "should handle genesis and payment" in withFuncSettings(currentFS.copy(blockVersion3AfterHeight = 3))(withGenerateSubscription() { d =>
      ...
      val genesisAmount = Constants.TotalTac * Constants.UnitsInTac + reward
    ...
    "should handle rollback properly" in {
      ...
      withGenerateSubscription() { d =>
        ...
        rollback.stateUpdate.balances shouldBe Seq(
          BalanceUpdate(TxHelpers.defaultAddress, Tac, 10000001036400000L, after = 10000000600000000L),
          BalanceUpdate(TxHelpers.defaultAddress, issue.asset, 2000, after = 0),
          BalanceUpdate(TxHelpers.secondAddress, Tac, 100000000, after = 0)
        )
      ...
      withGenerateSubscription() { d =>
        ...
        rollback.stateUpdate.balances shouldBe Seq(
          BalanceUpdate(TxHelpers.defaultAddress, Tac, 10000000935800000L, after = 10000001099400000L),
          BalanceUpdate(TxHelpers.defaultAddress, issue.asset, 2000, after = 0),
          BalanceUpdate(TxHelpers.secondAddress, Tac, 200000000, after = 100000000)
        )
  ...
  def withGenerateSubscription(request: SubscribeRequest = SubscribeRequest.of(1, Int.MaxValue))(generateBlocks: Domain => Unit)(
    ...
    withDomainAndRepo { (d, repo) =>
      d.appendBlock(TxHelpers.genesis(TxHelpers.defaultSigner.toAddress, Constants.TotalTac * Constants.UnitsInTac))
  ...
  def withSettings(settings: TacSettings)(f: => Unit): Unit = {
    val oldSettings = currentSettings
  ...
  implicit class ProtoSubscribeEventOps(e: com.tacplatform.events.api.grpc.protobuf.SubscribeEvent) {
