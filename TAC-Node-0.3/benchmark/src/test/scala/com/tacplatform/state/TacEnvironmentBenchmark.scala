...
import com.tacplatform.account.{AddressOrAlias, AddressScheme, Alias}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, EitherExt2}
import com.tacplatform.database.{LevelDBFactory, LevelDBWriter}
...
import com.tacplatform.state.TacEnvironmentBenchmark._
import com.tacplatform.state.bench.DataTestData
...
/**
  ...
  * 2. Import it: https://github.com/tacplatform/Tac/wiki/Export-and-import-of-the-blockchain#import-blocks-from-the-binary-file
...
class TacEnvironmentBenchmark {
  ...
  @Benchmark
  def accountBalanceOf_tac_test(st: AccountBalanceOfTacSt, bh: Blackhole): Unit = {
...
object TacEnvironmentBenchmark {
  ...
  class BaseSt {
    ...
    private val tacSettings: TacSettings = {
      val config = loadConfig(ConfigFactory.parseFile(new File(benchSettings.networkConfigFile)))
      TacSettings.fromRootConfig(config)
    }

    AddressScheme.current = new AddressScheme {
      override val chainId: Byte = tacSettings.blockchainSettings.addressSchemeCharacter.toByte
    }

    private val db: DB = {
      val dir = new File(tacSettings.dbSettings.directory)
      if (!dir.isDirectory) throw new IllegalArgumentException(s"Can't find directory at '${tacSettings.dbSettings.directory}'")
      LevelDBFactory.factory.open(dir, new Options)
    }

    val environment: Environment[Id] = {
      val state = LevelDBWriter.readOnly(db, tacSettings)
      new TacEnvironment(
