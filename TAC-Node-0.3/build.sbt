...
val langPublishSettings = Seq(
  ...
  homepage := Some(url("https://docs.tacplatform.com/en/technical-details/tac-contracts-language-description/maven-compiler-package.html")),
  developers := List(
    Developer("petermz", "Peter Zhelezniakov", "peterz@rambler.ru", url("https://tacplatform.com"))
  )
)
...
inScope(Global)(
  Seq(
    scalaVersion := "2.13.3",
    organization := "com.tacplatform",
    organizationName := "Tac Platform",
    V.fallback := (1, 3, 4),
    organizationHomepage := Some(url("https://tacplatform.com")),
    scmInfo := Some(ScmInfo(url("https://github.com/tacplatform/Tac"), "git@github.com:tacplatform/Tac.git", None)),
    licenses := Seq(("MIT", url("https://github.com/tacplatform/Tac/blob/master/LICENSE"))),
    scalacOptions ++= Seq(
      "-feature",
      "-deprecation",
      "-unchecked",
      ...
      "-opt:l:inline",
      "-opt-inline-from:**",
      "-Wconf:cat=deprecation&site=com.tacplatform.api.grpc.*:s" // Ignore gRPC warnings
    ),
    crossPaths := false,
...
packageAll := {
  ...
  IO.copyFile(nodeDebFile, new File(baseDirectory.value, "docker/target/tac.deb"))
