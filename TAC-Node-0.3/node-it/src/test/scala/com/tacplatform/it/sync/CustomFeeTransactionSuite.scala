...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.toTac(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""tac.fees.transfer.$assetId = 100000
                                                         |tac.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("tac.miner.enable=no").withFallback(minerConfig)
