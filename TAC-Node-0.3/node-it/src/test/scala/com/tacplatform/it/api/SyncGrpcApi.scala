...
import com.tacplatform.it.api.SyncHttpApi.RequestAwaitTime
...
import com.tacplatform.protobuf.block.Block.Header
import com.tacplatform.protobuf.block.{PBBlocks, VanillaBlock}
...
object SyncGrpcApi extends Assertions {
  ...
  implicit class NodeExtGrpc(n: Node) {
    ...
    import com.tacplatform.account.{Address => Addr}
    import com.tacplatform.it.api.AsyncGrpcApi.{NodeAsyncGrpcApi => async}
    ...
    def stateChanges(request: TransactionsRequest): Seq[(com.tacplatform.transaction.Transaction, StateChangesDetails)] = {
    ...
    def tacBalance(address: ByteString): TacBalances = {
      accounts.getBalances(BalancesRequest.of(address, Seq(ByteString.EMPTY))).next().getTac
    }
