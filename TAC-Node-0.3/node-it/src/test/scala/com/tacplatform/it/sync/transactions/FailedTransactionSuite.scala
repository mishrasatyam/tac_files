...
import com.tacplatform.api.http.ApiError.{ScriptExecutionError, TransactionNotAllowedByAccountScript, TransactionNotAllowedByAssetScript}
import com.tacplatform.api.http.DebugMessage
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.it.api.SyncHttpApi._
import com.tacplatform.it.api.{DebugStateChanges, TransactionStatus}
...
class FailedTransactionSuite extends BaseTransactionSuite with CancelAfterFailure with FailedTransactionSuiteLike[String] with OverflowBlock {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.transfer(sender.keyPair, contractAddress, 100.tac, minFee, waitForTx = true)
  ...
  test("InvokeScriptTransaction: insufficient action fees propagates failed transaction") {
    ...
    for (typeName <- Seq("transfer", "issue", "reissue", "burn")) {
      ...
      sendTxsAndThenPriorityTx(
        ...
        val text = s"Fee in TAC for InvokeScriptTransaction ($invokeFee in TAC)" +
          s"$scriptInvokedInfo$issuedInfo does not exceed minimal value of $minFee TAC."
  ...
  test("InvokeScriptTransaction: invoke script error in action asset propagates failed transaction") {
    val invokeFee            = 0.005.tac + smartFee
    val setAssetScriptMinFee = setAssetScriptFee + smartFee
    val priorityFee          = setAssetScriptMinFee + invokeFee
  ...
  test("InvokeScriptTransaction: sponsored fee on failed transaction should be charged correctly") {
    val invokeFee            = 0.005.tac + smartFee
    val invokeFeeInAsset     = invokeFee / 100000 // assetFee = feeInTac / feeUnit * sponsorship
    val setAssetScriptMinFee = setAssetScriptFee + smartFee
    val priorityFee          = setAssetScriptMinFee + invokeFee
  ...
  test("InvokeScriptTransaction: revalidate transactions returned to UTXPool because of `min-micro-block-age`") {
    ...
    sender.transfer(sender.keyPair, caller.toAddress.toString, 100.tac, minFee, waitForTx = true)
  ...
  private def exchangePreconditions(initScript: Option[String]): Precondition = {
    val transfers = Seq(
      sender.transfer(sender.keyPair, sellerAddress.toAddress.toString, 100.tac).id,
      sender.transfer(sender.keyPair, buyerAddress.toAddress.toString, 100.tac).id,
      sender.transfer(sender.keyPair, matcherAddress.toAddress.toString, 100.tac).id
    )
