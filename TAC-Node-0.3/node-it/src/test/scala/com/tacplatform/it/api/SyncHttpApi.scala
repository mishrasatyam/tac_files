...
import com.tacplatform.api.http.requests.IssueRequest
import com.tacplatform.api.http.{ApiError, DebugMessage}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.features.api.{ActivationStatus, FeatureActivationStatus}
import com.tacplatform.it.Node
import com.tacplatform.it.sync._
import com.tacplatform.lang.v1.compiler.Terms
import com.tacplatform.state.{AssetDistribution, AssetDistributionPage, DataEntry, Portfolio}
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    import com.tacplatform.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.tacplatform.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollbackWithoutBlacklisting(height: Int, returnToUTX: Boolean = true): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.tacplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def rollbackToBlockId(id: String): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.tacplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollbackToBlockId(id)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.tacplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
