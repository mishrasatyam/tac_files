...
import com.tacplatform.it.api._
...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_LONG, CONST_STRING}
...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("state changes order") {
    val script = ScriptCompiler
      ...
      .bytes()
      .base64
    sender.setScript(contract, Some(script), setScriptFee + 0.4.tac, waitForTx = true)
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.massTransfer(contract, List(Transfer(callerAddress, 3000), Transfer(recipientAddress, 3000)), 0.01.tac, assetId = Some(simpleAsset))
    sender.massTransfer(
      contract,
    ...
    val script = ScriptCompiler
      .compile(
        """
          ...
          |
          |@Callable(i)
          |func writeAndSendTac(value: Int, recipient: String, amount: Int) = {
          |    ScriptResult(
          |        WriteSet([DataEntry("result", value)]),
