...
import com.tacplatform.api.http.ApiError.{AssetDoesNotExist, TransactionDoesNotExist}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.it.BaseSuite
import com.tacplatform.it.api.{AssetInfo, BurnInfoResponse, IssueInfoResponse, ReissueInfoResponse, StateChangesDetails, Transaction}
import com.tacplatform.it.api.SyncHttpApi._
import com.tacplatform.it.sync._
import com.tacplatform.it.util._
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG}
...
class IssueReissueBurnAssetSuite extends BaseSuite {
  override val nodeConfigs: Seq[Config] =
    com.tacplatform.it.NodeConfigs.newBuilder
  ...
  for (method <- Seq(CallableMethod, TransactionMethod)) s"Asset Issue/Reissue/Burn via $method" - {
    ...
    for (data <- Seq(simpleNonreissuableAsset, simpleReissuableAsset, nftAsset, longMaxAsset, longMinAsset))
      s"${data.assetType} asset could be issued in callable" in {
        ...
        sender.balanceDetails(acc.toAddress.toString).regular shouldBe (initialTacBalance - setScriptPrice - fee)
      }
  ...
  def createDapp(scriptParts: String*): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.toAddress.toString, initialTacBalance, setScriptFee * 2, waitForTx = true)
  ...
  def invocationCost(aCount: Int, isSmartAcc: Boolean = true, sPCount: Int = 0, sAinActions: Int = 0): Long = {
    0.005.tac + (if (isSmartAcc) 0.004.tac else 0L) + 0.004.tac * sPCount + 0.004.tac * sAinActions + 1.tac * aCount
  }
