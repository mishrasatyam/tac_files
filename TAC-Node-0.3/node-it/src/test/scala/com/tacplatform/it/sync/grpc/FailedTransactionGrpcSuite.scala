...
import com.tacplatform.api.grpc.TransactionsRequest
...
import com.tacplatform.it.sync.transactions.FailedTransactionSuiteLike
...
import com.tacplatform.protobuf.transaction.{PBRecipients, PBSignedTransaction, PBTransactions, Recipient}
...
class FailedTransactionGrpcSuite extends GrpcBaseTransactionSuite with FailedTransactionSuiteLike[PBSignedTransaction] {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.broadcastTransfer(sender.keyPair, Recipient().withPublicKeyHash(contractAddr), 100.tac, minFee, waitForTx = true)
  ...
  test("InvokeScriptTransaction: sponsored fee on failed transaction should be charged correctly") {
    ...
    val prevBalance = sender.tacBalance(contractAddr).regular
    ...
    sendTxsAndThenPriorityTx(
      ...
      sender.tacBalance(contractAddr).regular shouldBe prevBalance - invokeFee * txs.size - priorityFee
      assertFailedTxs(txs)
    }
  }
  ...
  test("InvokeScriptTransaction: reject transactions if account script failed") {
    ...
    overflowBlock()
    val prevBalance = sender.tacBalance(callerAddr).regular
    sendTxsAndThenPriorityTx(
      _ =>
        sender.broadcastInvokeScript(
          caller,
      ...
      sender.tacBalance(callerAddr).regular shouldBe prevBalance - (txs.size - invalid.size) * invokeFee - priorityFee
      invalid
    }
  }
