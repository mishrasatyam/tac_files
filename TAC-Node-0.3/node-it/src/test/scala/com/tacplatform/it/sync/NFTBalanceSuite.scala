...
import com.tacplatform.it._
import com.tacplatform.it.api.AsyncHttpApi._
import com.tacplatform.it.api._
import com.tacplatform.it.transactions.BaseTransactionSuiteLike
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.toAddress.toString, 1000.tac, 0.001.tac)
  ...
  "after activation" - {
    ...
    "returns only nft with balance > 0 on /nft/{address}/limit/{limit}" in {
      ...
      val transfer = TransferTransaction
        .selfSigned(1.toByte, issuer, other.toAddress, randomTokenToTransfer, 1, Tac, 0.001.tac, ByteStr.empty, System.currentTimeMillis())
        .explicitGet()
    ...
    val transactions =
      Future.sequence(addrs map { addr =>
        NodeAsyncHttpApi(faucet)
          .transfer(faucet.address, addr, 1000.tac, 0.001.tac)
          .flatMap { tx =>
            NodeAsyncHttpApi(faucet)
