...
class AddressApiSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("balances for tac should be correct") {
    assertBalances(None)
  }
  ...
  test("limit violation requests should be handled correctly") {
    val limit     = miner.config.getInt("tac.rest-api.transactions-by-address-limit")
  ...
  override protected def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("tac.rest-api.transactions-by-address-limit = 20"))
      .withDefault(1)
