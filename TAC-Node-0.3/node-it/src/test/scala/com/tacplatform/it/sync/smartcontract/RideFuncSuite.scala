...
class RideFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(firstKeyPair, newAddress.toAddress.toString, 10.tac, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransaction.selfSigned(2.toByte, pkNewAddress, pkNewAddress.toAddress, Tac, 1.tac, Tac, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
          .explicitGet()
          .json()),
    ...
    sender.signedBroadcast(
      TransferTransaction.selfSigned(2.toByte, firstKeyPair, pkNewAddress.toAddress, IssuedAsset(ByteStr.decodeBase58(asset).get), 100000000, Tac, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
        .explicitGet()
        .json(),
      waitForTx = true
    )
    ...
    val udpatedScript =
      s"""
         |match tx {
         |  case _: SetScriptTransaction => true
         |  case _ => assetBalance(tx.sender, base58'$asset') >= 900000000 && tacBalance(tx.sender) >500000000
         |}
      """.stripMargin
    ...
    sender.signedBroadcast(
      TransferTransaction.selfSigned(2.toByte, firstKeyPair, pkNewAddress.toAddress, IssuedAsset(ByteStr.decodeBase58(asset).get), 800000000, Tac, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
        .explicitGet()
        .json(),
      waitForTx = true
    )
  ...
  test("split around empty string") {
    ...
    val transfer = TransferTransaction.selfSigned(2.toByte, pkNewAddress, pkNewAddress.toAddress, Tac, 1.tac, Tac, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
  ...
  test("lastBlock and blockInfoByHeight(last) must return liquid block") {
    ...
    val transfer = sender.transfer(newAddress, newAddress.toAddress.toString, 1.tac, minFee + (2 * smartFee))
    nodes.waitForHeightAriseAndTxPresent(transfer.id)
  }
}
