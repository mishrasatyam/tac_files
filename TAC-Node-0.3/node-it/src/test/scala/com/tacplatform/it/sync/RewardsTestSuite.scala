...
import com.tacplatform.api.http.ApiError.CustomValidationError
...
import com.tacplatform.it.{Node, ReportingTestName}
...
class RewardsTestSuite
  ...
  "reward changes accordingly node's votes and miner's balance changes by reward amount after block generation" - {
    "when miner votes for increase" in {
      ...
      rewardAtActivation.totalTacAmount shouldBe InitialAmount + initial
      ...
      rewardAfterFirstVote.totalTacAmount shouldBe InitialAmount + BigInt(initial) * BigInt(votingStartHeight - activationHeight + 1)
      ...
      rewardAtTermEnd.totalTacAmount shouldBe amountAfterTerm
      ...
      rewardSecVoting.totalTacAmount shouldBe amountAfterTerm + newReward * BigInt(secondVotingStartHeightPlusTwo - termEndHeight)
    }
