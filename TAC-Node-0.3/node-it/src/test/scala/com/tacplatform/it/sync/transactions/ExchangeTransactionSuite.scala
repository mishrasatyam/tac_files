...
import com.tacplatform.api.http.ApiError.{CustomValidationError, StateCheckFailed}
...
import com.tacplatform.it.sync.smartcontract.exchangeTx
import com.tacplatform.it.transactions.BaseTransactionSuite
import com.tacplatform.it.util._
import com.tacplatform.it.{NTPTime, NodeConfigs}
...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    val pair = AssetPair.createAssetPair("TAC", exchAsset.id().toString).get
  ...
  test("exchange tx with orders v3") {
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
           (1: Byte, 3: Byte, Tac, IssuedAsset(assetId)),
           (1: Byte, 3: Byte, Tac, Tac),
           (2: Byte, 3: Byte, Tac, IssuedAsset(assetId)),
           (3: Byte, 1: Byte, IssuedAsset(assetId), Tac),
           (2: Byte, 3: Byte, Tac, Tac),
           (3: Byte, 2: Byte, IssuedAsset(assetId), Tac)
         )) {
      ...
      val assetPair  = AssetPair.createAssetPair("TAC", assetId.toString).get
  ...
  test("exchange tx with orders v4 can use price that is impossible for orders v3/v2/v1") {
    ...
    sender.transfer(sender.keyPair, firstAddress, 1000.tac, waitForTx = true)
    ...
    val expirationTimestamp = ts + Order.MaxLiveTime
    val amount              = 1
    val nftTacPrice       = 1000 * math.pow(10, 8).toLong
    val nftForAssetPrice    = 1 * math.pow(10, 8).toLong

    val nftTacPair      = AssetPair.createAssetPair(nftAsset, "TAC").get
    val nftOtherAssetPair = AssetPair.createAssetPair(nftAsset, dec6AssetId).get

    val sellNftForTac =
      Order.sell(4.toByte, seller, matcher.publicKey, nftTacPair, amount, nftTacPrice, ts, expirationTimestamp, matcherFee, Tac)
    val buyNftForTac =
      Order.buy(4.toByte, buyer, matcher.publicKey, nftTacPair, amount, nftTacPrice, ts, expirationTimestamp, matcherFee, Tac)

    val sellNftForOtherAsset =
      Order.sell(4.toByte, buyer, matcher.publicKey, nftOtherAssetPair, amount, nftForAssetPrice, ts, expirationTimestamp, matcherFee, Tac)
    val buyNftForOtherAsset =
      Order.buy(4.toByte, seller, matcher.publicKey, nftOtherAssetPair, amount, nftForAssetPrice, ts, expirationTimestamp, matcherFee, Tac)
    ...
    val tx =
      ExchangeTransaction
        .signed(
          ...
          matcher = matcher.privateKey,
          order1 = buyNftForTac,
          order2 = sellNftForTac,
          amount = amount,
          price = nftTacPrice,
          buyMatcherFee = (BigInt(matcherFee) * amount / sellNftForTac.amount).toLong,
          sellMatcherFee = (BigInt(matcherFee) * amount / sellNftForTac.amount).toLong,
          fee = matcherFee,
          timestamp = ntpTime.correctedTime()
        )
        .explicitGet()
    ...
    sender.balanceDetails(sellerAddress).regular shouldBe sellerBalance + nftTacPrice - matcherFee
    sender.balanceDetails(buyerAddress).regular shouldBe buyerBalance - nftTacPrice - matcherFee
