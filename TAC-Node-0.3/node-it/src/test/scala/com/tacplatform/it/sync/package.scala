...
import com.tacplatform.it.api.SyncHttpApi.AssertiveApiError
...
import com.tacplatform.protobuf.transaction.{DataTransactionData, PBTransactions}
import com.tacplatform.state.DataEntry
import com.tacplatform.transaction.assets.IssueTransaction
import com.tacplatform.transaction.smart.script.ScriptCompiler

package object sync {
  val smartFee: Long                   = 0.004.tac
  val minFee: Long                     = 0.001.tac
  val leasingFee: Long                 = 0.001.tac
  val issueFee: Long                   = 1.tac
  val reissueFee: Long                 = 1.tac
  val reissueReducedFee: Long          = 0.001.tac
  val burnFee: Long                    = 0.001.tac
  val invokeFee: Long                  = 0.009.tac
  val sponsorFee: Long                 = 1.tac
  val sponsorReducedFee: Long          = 0.001.tac
  val setAssetScriptFee: Long          = 1.tac
  val setScriptFee: Long               = 0.01.tac
  val transferAmount: Long             = 10.tac
  val leasingAmount: Long              = transferAmount
  val issueAmount: Long                = transferAmount
  val massTransferFeePerTransfer: Long = 0.0005.tac
  val someAssetAmount: Long            = 9999999999999L
  val matcherFee: Long                 = 0.003.tac
  val orderFee: Long                   = matcherFee
  val smartMatcherFee: Long            = 0.007.tac
