...
import com.tacplatform.api.grpc.{ApplicationStatus, TransactionsByIdRequest, TransactionStatus => PBTransactionStatus}
import com.tacplatform.api.http.ApiError.TransactionDoesNotExist
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.it.api.TransactionStatus
...
trait FailedTransactionSuiteLike[T] extends ScorexLogging { _: Matchers =>
  ...
  def sendTxsAndThenPriorityTx[S](t: Int => T, pt: () => T)(
    ...
    val maxTxsInMicroBlock = sender.config.getInt("tac.miner.max-transactions-in-micro-block")
...
object FailedTransactionSuiteLike {
  ...
  val configForMinMicroblockAge: Config = ConfigFactory.parseString(s"""
     |tac.miner.min-micro-block-age = 7
     |tac.miner.max-transactions-in-micro-block = 1
     |""".stripMargin)

  val Configs: Seq[Config] =
    NodeConfigs.newBuilder
      .overrideBase(_.quorum(0))
      .overrideBase(_.raw(s"tac.miner.max-transactions-in-micro-block = 50"))
      .withDefault(1)
