...
import com.tacplatform.account.{AddressScheme, Alias, KeyPair}
import com.tacplatform.api.grpc.BalanceResponse.TacBalances
import com.tacplatform.api.grpc.{TransactionStatus => PBTransactionStatus, _}
...
import com.tacplatform.lang.script.{Script => Scr}
import com.tacplatform.lang.v1.Serde
...
object AsyncGrpcApi {
  implicit class NodeAsyncGrpcApi(val n: Node) {
    ...
    import com.tacplatform.protobuf.transaction.{Transaction => PBTransaction, _}
    ...
    ): Future[Seq[(com.tacplatform.transaction.Transaction, StateChangesDetails)]] = {
      val (obs, result) = createCallObserver[TransactionResponse]
      transactions.getTransactions(request, obs)
      result.runToFuture.map { r =>
        import com.tacplatform.state.{InvokeScriptResult => VISR}
        r.map { r =>
    ...
    def broadcastTransfer(
      ...
      val unsigned = PBTransaction(
        chainId,
        ByteString.copyFrom(source.publicKey.arr),
        Some(Amount.of(if (feeAssetId == "TAC") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(feeAssetId)), fee)),
        timestamp,
        version,
        PBTransaction.Data.Transfer(
          TransferTransactionData.of(
            Some(recipient),
            Some(Amount.of(if (assetId == "TAC") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(assetId)), amount)),
            attachment
          )
        )
      )
    ...
    def exchange(
        ...
        timestamp: Long,
        version: Byte,
        matcherFeeAssetId: String = "TAC"
    ): Future[PBSignedTransaction] = {

      val unsigned = PBTransaction(
        chainId,
        ByteString.copyFrom(matcher.publicKey.arr),
        Some(Amount.of(if (matcherFeeAssetId == "TAC") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(matcherFeeAssetId)), fee)),
        timestamp,
        version,
    ...
    def tacBalance(address: ByteString): Future[TacBalances] = {
      ...
      result.map(_.headOption.getOrElse(throw new NoSuchElementException("Balances not found for address")).getTac).runToFuture
    }
    ...
    def updateAssetInfo(
      ...
      val unsigned = PBTransaction(
        chainId,
        ByteString.copyFrom(sender.publicKey.arr),
        Some(Amount.of(if (feeAsset == Tac) ByteString.EMPTY else ByteString.copyFrom(Base58.decode(feeAsset.maybeBase58Repr.get)), fee)),
        System.currentTimeMillis(),
        version,
