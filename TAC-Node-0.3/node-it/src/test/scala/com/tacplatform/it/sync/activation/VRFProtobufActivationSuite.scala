...
class VRFProtobufActivationSuite extends BaseTransactionSuite {
  ...
  override protected def nodeConfigs: Seq[Config] =
    NodeConfigs
      ...
      .overrideBase(_.raw(s"tac.blockchain.custom.functionality.min-asset-info-update-interval = $updateInterval"))
      .buildNonConflicting()
  ...
  test("able to broadcast tx of new version after activation") {
    val senderTacBalance    = sender.balanceDetails(senderAcc.toAddress.toString)
    val recipientTacBalance = sender.balanceDetails(recipientAcc.toAddress.toString)
    sender.transfer(senderAcc, recipientAcc.toAddress.toString, transferAmount, version = TxVersion.V3, waitForTx = true)

    sender.balanceDetails(senderAcc.toAddress.toString).available shouldBe senderTacBalance.available - transferAmount - minFee
    sender.balanceDetails(recipientAcc.toAddress.toString).available shouldBe recipientTacBalance.available + transferAmount
  }
  ...
  private def mkOrders: (Order, Order) = {
    ...
    val buyOrder = Order.buy(
      ...
      sender.keyPair,
      sender.publicKey,
      AssetPair.createAssetPair("TAC", assetId).get,
      amount,
      price,
      ts,
