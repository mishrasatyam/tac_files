...
class SetAssetScriptGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("sender's tac balance is decreased by fee") {
    ...
    sender.tacBalance(firstAddress).available shouldBe firstBalance - setAssetScriptFee
    sender.tacBalance(firstAddress).effective shouldBe firstEffBalance - setAssetScriptFee
  }

  test("not able set script without having enough tac") {
    val firstBalance    = sender.tacBalance(firstAddress).available
    val firstEffBalance = sender.tacBalance(firstAddress).effective
    assertGrpcError(
  ...
  test("try to make SetAssetScript for asset v1") {
    ...
    val balance    = sender.tacBalance(thirdAddress).available
    val effBalance = sender.tacBalance(thirdAddress).effective
    ...
    sender.tacBalance(thirdAddress).available shouldBe balance
    sender.tacBalance(thirdAddress).effective shouldBe effBalance
