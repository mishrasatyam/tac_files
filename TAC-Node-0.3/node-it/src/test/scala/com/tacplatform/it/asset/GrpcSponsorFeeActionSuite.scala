...
class GrpcSponsorFeeActionSuite extends FreeSpec with GrpcBaseTransactionSuiteLike {
  ...
  "State changes" - {
    ...
    "Use sponsored asset as fee" in {
      ...
      miner.balance(dAppAddress).balance shouldBe startDAppBalance - dAppTacOutgo
      miner.balance(miner.address).balance shouldBe startMinerBalance + dAppTacOutgo + blockReward
    }
  ...
  private def createDApp(script: String, address: KeyPair = miner.createKeyPair()): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.publicKey.toAddress.toString, initialTacBalance, minFee, waitForTx = true)
