...
class TransferTxFromProtoSuite extends BaseTransactionSuite {
  ...
  private val scriptText =
    s"""
      ...
      |    let assetId = if (!transferTx.assetId.isDefined()) then {"TAC"} else {transferTx.assetId.value().toBase58String()}
      |    let feeAssetId = if (!transferTx.feeAssetId.isDefined()) then {"TAC"} else {transferTx.feeAssetId.value().toBase58String()}
      |[
  ...
  test("TransferTransaction with Tac from proto bytes") {
    ...
    val transferTx = TransferTransaction
      .selfSigned(
        ...
        feeAsset = Tac,
        fee = minFee,
        attachment = ByteStr("TAC transfer".getBytes),
    ...
    sender.getDataByKey(dAppAddress, "assetId").value shouldBe "TAC"
    sender.getDataByKey(dAppAddress, "feeAssetId").value shouldBe "TAC"
    sender.getDataByKey(dAppAddress, "attachment").value shouldBe Base58.encode("TAC transfer".getBytes)
