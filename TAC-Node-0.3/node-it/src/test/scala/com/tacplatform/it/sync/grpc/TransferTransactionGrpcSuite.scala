...
class TransferTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime {
  ...
  test("asset transfer changes sender's and recipient's asset balance by transfer amount and tac by fee") {
    for (v <- transferTxSupportedVersions) {
      ...
      sender.tacBalance(firstAddress).available shouldBe firstBalance - minFee
      sender.tacBalance(firstAddress).effective shouldBe firstEffBalance - minFee
  ...
  test("tac transfer changes tac balances and eff.b. by transfer amount and fee") {
    for (v <- transferTxSupportedVersions) {
      val firstBalance     = sender.tacBalance(firstAddress).available
      val firstEffBalance  = sender.tacBalance(firstAddress).effective
      val secondBalance    = sender.tacBalance(secondAddress).available
      ...
      sender.tacBalance(firstAddress).available shouldBe firstBalance - transferAmount - minFee
      sender.tacBalance(firstAddress).effective shouldBe firstEffBalance - transferAmount - minFee
      sender.tacBalance(secondAddress).available shouldBe secondBalance + transferAmount
      sender.tacBalance(secondAddress).effective shouldBe secondEffBalance + transferAmount
    }
  }
  ...
  test("can not make transfer without having enough tac balance") {
    for (v <- transferTxSupportedVersions) {
      val firstBalance     = sender.tacBalance(firstAddress).available
      val firstEffBalance  = sender.tacBalance(firstAddress).effective
      val secondBalance    = sender.tacBalance(secondAddress).available
