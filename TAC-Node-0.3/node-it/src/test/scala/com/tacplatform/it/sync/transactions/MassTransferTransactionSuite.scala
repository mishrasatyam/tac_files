...
import com.tacplatform.api.http.requests.{MassTransferRequest, SignedMassTransferRequest}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.it.api.MassTransferTransactionInfo
...
import com.tacplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
...
class MassTransferTransactionSuite extends BaseTransactionSuite {
  ...
  test("reporting MassTransfer transactions") {
    for (v <- massTransferTxSupportedVersions) {
      val transfers = List(Transfer(firstAddress, 5.tac), Transfer(secondAddress, 2.tac), Transfer(thirdAddress, 3.tac))
      ...
      assert((txSender \ "totalAmount").as[Long] == 10.tac)
      ...
      assert((txRecipient \ "totalAmount").as[Long] == 10.tac)
  ...
  test("reporting MassTransfer transactions to aliases") {
    for (v <- massTransferTxSupportedVersions) {
      ...
      val transfers = aliases.map { alias =>
        Transfer(Alias.create(alias).explicitGet().stringRepr, 2.tac)
      }
      val txId = sender.massTransfer(firstKeyPair, transfers, 300000, version = v).id
      nodes.waitForHeightAriseAndTxPresent(txId)
