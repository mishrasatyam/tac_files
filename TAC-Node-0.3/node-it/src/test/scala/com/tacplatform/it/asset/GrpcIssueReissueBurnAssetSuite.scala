...
import com.tacplatform.api.grpc.AssetInfoResponse
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, EitherExt2}
import com.tacplatform.it.api.SyncGrpcApi._
import com.tacplatform.it.api.{BurnInfoResponse, IssueInfoResponse, ReissueInfoResponse, StateChangesDetails}
import com.tacplatform.it.sync._
import com.tacplatform.it.sync.grpc.GrpcBaseTransactionSuiteLike
import com.tacplatform.it.util._
import com.tacplatform.lang.v1.FunctionHeader
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, FUNCTION_CALL}
import com.tacplatform.lang.v1.estimator.v2.ScriptEstimatorV2
import com.tacplatform.protobuf.transaction.{PBRecipients, PBTransactions}
...
class GrpcIssueReissueBurnAssetSuite extends FreeSpec with GrpcBaseTransactionSuiteLike {
  ...
  private val setScriptPrice      = 0.01.tac
  ...
  for (method <- Seq(CallableMethod, TransactionMethod)) s"Asset Issue/Reissue/Burn via $method" - {
    ...
    for (data <- Seq(simpleNonreissuableAsset, simpleReissuableAsset, nftAsset, longMaxAsset, longMinAsset))
      s"${data.assetType} asset could be issued in callable" in {
        ...
        sender.tacBalance(acc).regular shouldBe (initialTacBalance - setScriptPrice - fee)
      }
  ...
  def createDapp(scriptParts: String*): KeyPair = {
    ...
    miner.broadcastTransfer(sender.keyPair, PBRecipients.create(address.toAddress), initialTacBalance, minFee, waitForTx = true)
  ...
  def invocationCost(issuesCount: Int, isSmartAcc: Boolean = true, smartPaymentCount: Int = 0, smartAssetsInActions: Int = 0): Long = {
    0.005.tac + (if (isSmartAcc) 0.004.tac else 0L) + 0.004.tac * smartPaymentCount + 0.004.tac * smartAssetsInActions + 1.tac * issuesCount
  }
