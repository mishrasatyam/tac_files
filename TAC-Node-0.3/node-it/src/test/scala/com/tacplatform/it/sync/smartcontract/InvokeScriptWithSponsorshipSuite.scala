...
import com.tacplatform.it.sync.{minFee, smartMinFee}
...
class InvokeScriptWithSponsorshipSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send tac to dApp and caller accounts") {
    sender.transfer(sender.keyPair, dAppAddress, 5.tac, minFee, waitForTx = true).id
    sender.transfer(sender.keyPair, callerAddress, 5.tac, minFee, waitForTx = true).id
  ...
  test("required fee in sponsored assets considers scripts count") {
    ...
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 TAC or $feeAmount"
    )
    ...
    sender.accountBalances(dAppAddress)._1 shouldBe dAppInitBalance - 0.009.tac - 0.053.tac
  ...
  test("dApp caller is dApp address") {
    ...
    val dAppTacBalance = sender.accountBalances(dAppAddress)._1
    ...
    sender.accountBalances(dAppAddress)._1 shouldBe dAppTacBalance - 0.009.tac
  }
