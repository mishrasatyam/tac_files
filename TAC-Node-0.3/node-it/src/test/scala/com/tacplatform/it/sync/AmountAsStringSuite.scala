...
import com.tacplatform.it.sync.transactions.OverflowBlock
import com.tacplatform.it.transactions.BaseTransactionSuite
import com.tacplatform.state.IntegerDataEntry
...
import com.tacplatform.transaction.{CreateAliasTransaction, TxVersion}
...
class AmountAsStringSuite extends BaseTransactionSuite with OverflowBlock {
  ...
  test("amount as string in exchange transaction") {
    ...
    val buyOrder = Order.buy(
      ...
      exchanger,
      exchanger.publicKey,
      AssetPair.createAssetPair("TAC", exchAssetId).get,
      amount,
      price,
      ts,
  ...
  test("amount as string in rewards api") {
    ...
    rewards.totalTacAmount shouldBe rewardsAsInteger.totalTacAmount
    rewards.currentReward shouldBe rewardsAsInteger.currentReward
    rewards.minIncrement shouldBe rewardsAsInteger.minIncrement
    rewardsByHeight.totalTacAmount shouldBe rewardsAsInteger.totalTacAmount
  ...
  test("amount as string in debug api") {
    ...
    val stateTacOnHeight = sender.getWithCustomHeader(
      s"/debug/stateTac/${sender.height}",
      headerValue = "application/json;large-significand-format=string",
      withApiKey = true
    )
    (parseResponse(stateTacOnHeight) \ firstAddress).get shouldBe JsString(firstBalance.toString)
  }
