package com.tacplatform.it.account.storage
...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING}
...
class RemoveEntrySuite extends BaseSuite {
  ...
  "Remove entry from account storage" - {
    for (data <- Seq(stringTestData, integerTestData, booleanTestData, binaryTestData)) s"${data.ct}Entry could be removed from account storage" in {
      ...
      miner.getDataByKey(address, data.k).getClass.getCanonicalName shouldBe s"com.tacplatform.state.${data.ct}DataEntry"
    ...
    "Could remove 100 entries" in {
      ...
      miner.putData(keyPair, data.toList, 1.tac, true)
  ...
  def createDapp(scriptParts: String*): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.toAddress.toString, 10.tac, minFee, waitForTx = true)
