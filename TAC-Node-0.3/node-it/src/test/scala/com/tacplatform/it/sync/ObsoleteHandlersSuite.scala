...
import com.tacplatform.api.http.requests.{CreateAliasRequest, DataRequest, LeaseCancelRequest, LeaseRequest, MassTransferRequest, SponsorFeeRequest, TransferRequest}
...
class ObsoleteHandlersSuite extends BaseTransactionSuite with BeforeAndAfterAll {
  ...
  test("assets masstransfer") {
    ...
    val transfers = List(Transfer(secondAddress, 1.tac), Transfer(thirdAddress, 2.tac))
