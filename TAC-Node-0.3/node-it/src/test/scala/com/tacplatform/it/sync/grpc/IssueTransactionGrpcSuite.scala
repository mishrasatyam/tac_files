...
class IssueTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime with TableDrivenPropertyChecks {
  ...
  test("asset issue changes issuer's asset balance") {
    for (v <- issueTxSupportedVersions) {
      ...
      val issuerBalance    = sender.tacBalance(issuerAddress).available
      val issuerEffBalance = sender.tacBalance(issuerAddress).effective
      ...
      sender.tacBalance(issuerAddress).available shouldBe issuerBalance - issueFee
      sender.tacBalance(issuerAddress).effective shouldBe issuerEffBalance - issueFee
  ...
  test("not able to issue asset with fee less then issueFee (minFee for NFT)") {
    for (v <- issueTxSupportedVersions) {
      ...
      sender.tacBalance(issuerAddress).available shouldBe issuerBalance
      sender.tacBalance(issuerAddress).effective shouldBe issuerEffBalance
    }
  }
  ...
  test("Not able to create asset when insufficient funds") {
    ...
    val issuerEffBalance = sender.tacBalance(issuerAddress).effective
    val bigAssetFee      = issuerEffBalance + 1.tac
