...
import com.tacplatform.api.http.ApiError.{Mistiming, StateCheckFailed, WrongJson}
...
import com.tacplatform.it.sync.{script, someAssetAmount, _}
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough tac") {
    val (balance, eff) = miner.accountBalances(firstAddress)
    for (v <- setAssetScrTxSupportedVersions) {
