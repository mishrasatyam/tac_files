...
import com.tacplatform.account.{AddressScheme, KeyPair}
import com.tacplatform.api.http.ApiError.{CustomValidationError, TooBigArrayAllocation}
...
import com.tacplatform.it.api.{TransactionInfo, UnexpectedStatusCodeException}
import com.tacplatform.it.sync.{calcDataFee, minFee, _}
...
import com.tacplatform.state.{BinaryDataEntry, BooleanDataEntry, DataEntry, EmptyDataEntry, IntegerDataEntry, StringDataEntry}
import com.tacplatform.transaction.smart.script.ScriptCompiler
import com.tacplatform.transaction.{DataTransaction, Proofs, TxVersion}
...
class DataTransactionSuite extends BaseTransactionSuite with EitherValues {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.postForm("/addresses")
    sender.transfer(firstKeyPair, fourthAddress, 10.tac, minFee, waitForTx = true)
  }

  test("should not put 65-sized proof") {
    val keyPair = sender.createKeyPair()
    sender.transfer(sender.keyPair, keyPair.toAddress.stringRepr, 1.tac, waitForTx = true)
    sender.setScript(
      keyPair,
      Some(
        ScriptCompiler
  ...
  test("cannot broadcast data without having enough tac") {
    for (v <- dataTxSupportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
  ...
  test("try to send tx above limits of key, value and entries count") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, extraValueData, 1.tac, version = v), TooBig)
      nodes.waitForHeightArise()

      val largeBinData = List.tabulate(5)(n => BinaryDataEntry(extraKey, ByteStr(Array.fill(maxValueSize)(n.toByte))))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, largeBinData, 1.tac, version = v), TooBig)
      nodes.waitForHeightArise()

      val largeStrData = List.tabulate(5)(n => StringDataEntry(extraKey, "A" * maxValueSize))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, largeStrData, 1.tac, version = v), TooBig)
      nodes.waitForHeightArise()

      val tooManyEntriesData = List.fill(maxEntryCount + 1)(IntegerDataEntry("key", 88))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, tooManyEntriesData, 1.tac, version = v), TooBig)
      nodes.waitForHeightArise()
    }
  }
  ...
  test("put data in liquid block") {
    ...
    sender.transfer(firstKeyPair, newAddress.toAddress.toString, 2 tac, 1 tac, waitForTx = true)
    sender.broadcastData(newAddress, entries, 0.1 tac, waitForTx = true)
