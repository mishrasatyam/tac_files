...
import com.tacplatform.it.sync.smartcontract.RideV4ActivationSuite._
...
class InvokeScriptTransactionRideV5Suite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.massTransfer(
      callerPK,
      List(
        Transfer(dAppV3, 10.tac),
        Transfer(dAppV4, 10.tac),
        Transfer(dAppV5, 10.tac)
      ),
      1.tac,
      waitForTx = true
    )

    sender.createAlias(dAppV3PK, dAppAliasV3, fee = 1.tac)
    sender.createAlias(dAppV4PK, dAppAliasV4, fee = 1.tac)
    sender.createAlias(dAppV5PK, dAppAliasV5, fee = 1.tac)
