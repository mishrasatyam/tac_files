...
import com.tacplatform.protobuf.transaction.{PBSignedTransaction, PBTransactions, Recipient}
...
class InvokeScriptPayAndTransferAssetGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("set script to dApp account and transfer out all tac") {
    val dAppBalance = sender.tacBalance(dAppAddress)
  ...
  test("dApp can transfer payed asset if its own balance is 0") {
    val dAppInitBalance     = sender.tacBalance(dAppAddress)
    val callerInitBalance   = sender.tacBalance(callerAddress)
    val receiverInitBalance = sender.tacBalance(receiverAddress)

    val paymentAmount = 10

    invoke("resendPayment", paymentAmount, assetId)

    sender.tacBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.tacBalance(callerAddress).regular shouldBe callerInitBalance.regular - smartMinFee
    sender.tacBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can transfer payed smart asset if its own balance is 0") {
    ...
    sender.tacBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.tacBalance(callerAddress).regular shouldBe callerInitBalance.regular - fee
    sender.tacBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can't transfer payed smart asset if it rejects transfers and its own balance is 0") {
    ...
    sender.tacBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.tacBalance(callerAddress).regular shouldBe callerInitBalance.regular
    sender.tacBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can transfer payed Tac if its own balance is 0") {
    ...
    sender.tacBalance(dAppAddress).regular shouldBe dAppInitBalance.regular + paymentAmount - 1
    sender.tacBalance(callerAddress).regular shouldBe callerInitBalance.regular - paymentAmount - smartMinFee
    sender.tacBalance(receiverAddress).regular shouldBe receiverInitBalance.regular + 1
  }

  def invoke(func: String, amount: Long, assetId: String = "TAC", fee: Long = 500000): PBSignedTransaction = {
    val assetIdByteSting = if (assetId == "TAC") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(assetId))
