...
object NodeConfigs {
  ...
  object Templates {
    ...
    def quorum(n: Int): String = s"tac.miner.quorum = $n"
    def preactivatedFeatures(f: (Int, Int)*): String = {
      s"""
         |tac.blockchain.custom.functionality.pre-activated-features {
         ${f.map {case (id, height) => s"|  $id = $height"}.mkString("\n")}
         |}""".stripMargin
    }
    def minAssetInfoUpdateInterval(blocks: Int): String =
      s"tac.blockchain.custom.functionality.min-asset-info-update-interval = $blocks"

    val nonMiner: String = "tac.miner.enable = no"
  }
