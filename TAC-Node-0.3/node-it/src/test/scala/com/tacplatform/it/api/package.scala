...
import com.tacplatform.utils.{Paged, ScorexLogging}
...
package object api {
  ...
  implicit val addressReads: Reads[com.tacplatform.account.Address] = Reads {
  ...
  implicit val dstMapReads: Reads[Map[com.tacplatform.account.Address, Long]] = Reads { json =>
    json.validate[Map[String, Long]].map { dst =>
      dst.map {
        case (addrStr, balance) =>
          com.tacplatform.account.Address.fromString(addrStr).explicitGet() -> balance
      }
    }
  }
  ...
  implicit val distributionReads: Reads[AssetDistribution] = Reads { json =>
    json
      .validate[Map[com.tacplatform.account.Address, Long]]
  ...
  implicit val distributionPageReads: Reads[AssetDistributionPage] = Reads { json =>
    json.validate[Paged[com.tacplatform.account.Address, AssetDistribution]].map(pg => AssetDistributionPage(pg))
  }
}
