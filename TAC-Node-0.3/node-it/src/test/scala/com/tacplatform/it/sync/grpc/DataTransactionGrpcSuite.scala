...
import com.tacplatform.it.sync.{dataTxSupportedVersions, minFee}
...
import com.tacplatform.protobuf.transaction.{DataTransactionData, PBRecipients, PBTransactions, Recipient}
import com.tacplatform.state.StringDataEntry
import com.tacplatform.transaction.smart.script.ScriptCompiler
import com.tacplatform.transaction.{DataTransaction, TxVersion}
...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  protected override def beforeAll(): Unit = {
    super.beforeAll()
    sender.broadcastTransfer(firstAcc, Recipient().withPublicKeyHash(fourthAddress), 10.tac, minFee, waitForTx = true)
  }

  test("should not put 65-sized proof") {
    val keyPair = sender.generateKeyPair()
    sender.broadcastTransfer(sender.keyPair, PBRecipients.create(keyPair.toAddress), 1.tac, 100000L, waitForTx = true)
    sender.setScript(
      keyPair,
      Right(
        Some(
  ...
  test("sender's tac balance is decreased by fee.") {
    for (v <- dataTxSupportedVersions) {
      val firstBalance    = sender.tacBalance(firstAddress).available
      val firstEffBalance = sender.tacBalance(firstAddress).effective
      ...
      sender.tacBalance(firstAddress).available shouldBe firstBalance - fee
      sender.tacBalance(firstAddress).effective shouldBe firstEffBalance - fee
    }
  }

  test("cannot put data without having enough tac") {
    for (v <- dataTxSupportedVersions) {
      val firstBalance    = sender.tacBalance(firstAddress).available
      val firstEffBalance = sender.tacBalance(firstAddress).effective
      ...
      sender.tacBalance(firstAddress).available shouldBe firstBalance
      sender.tacBalance(firstAddress).effective shouldBe firstEffBalance
    }
  }
  ...
  test("data definition and retrieval") {
    for (v <- dataTxSupportedVersions) {
      ...
      val firstBalance       = sender.tacBalance(txSenderAddress).available
      val firstEffBalance    = sender.tacBalance(txSenderAddress).effective
      ...
      sender.tacBalance(txSenderAddress).available shouldBe firstBalance - fee
      sender.tacBalance(txSenderAddress).effective shouldBe firstEffBalance - fee
    }
  }
  ...
  test("try to send tx above limits of key, value and size") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertGrpcError(
        sender.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.tac, version = v),
        "Empty key found",
        Code.INVALID_ARGUMENT
      )
      assertGrpcError(
