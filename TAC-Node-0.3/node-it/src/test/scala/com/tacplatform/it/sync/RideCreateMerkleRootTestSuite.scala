...
import com.tacplatform.common.merkle.Merkle
...
import com.tacplatform.it.{Node, NodeConfigs, ReportingTestName, TransferSending}
    ...
    def tt(tx: Transaction) =
      TransferTransaction
        .create(
          ...
          Tac /* not support tx.asset.fold(Tac)(v => IssuedAsset(Base58.decode(v))) */,
          tx.amount.get,
          Tac /* not support tx.feeAsset.fold(Tac)(v => Issued(Base58.decode(v))) */,
          tx.fee,
