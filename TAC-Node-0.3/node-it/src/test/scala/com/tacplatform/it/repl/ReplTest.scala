package com.tacplatform.it.repl
...
import com.tacplatform.it.sync.transactions.{FailedTransactionSuiteLike, OverflowBlock}
...
class ReplTest extends BaseTransactionSuite with FailedTransactionSuiteLike[String] with OverflowBlock {
  ...
  test("tac context") {
    ...
    val trans  = miner.transfer(miner.keyPair, issuer.toAddress.toString, 100.tac, 1.tac, version = TxVersion.V3, waitForTx = true)
    miner.transfer(miner.keyPair, sample.toAddress.toString, 100.tac, 1.tac, waitForTx = true)
    ...
    miner.setScript(issuer, Some(failDApp), 1.tac, waitForTx = true)
    ...
    await(repl.execute(s""" tacBalance(Address(base58'${sample.toAddress}')).regular """)) shouldBe Right(s"res12: Int = ${100.tac}")
    await(repl.execute(""" this.tacBalance() """))
