...
import com.tacplatform.it.BaseSuite
import com.tacplatform.it.api.SyncHttpApi._
import com.tacplatform.it.api.{IssueInfoResponse, SponsorFeeResponse, StateChangesDetails}
...
import com.tacplatform.state.Sponsorship
...
class SponsorFeeActionSuite extends BaseSuite {
  private val initialTacBalance = 100.tac
  ...
  "State changes" - {
    ...
    "Use sponsored asset as fee" in {
      ...
      val dAppTacOutgo = smartMinFee + Sponsorship.toTac(assetFee, minSponsoredAssetFee)
      ...
      miner.balanceAtHeight(globalDAppAddress, lastCheckHeight) shouldBe startDAppBalance - dAppTacOutgo

      miner.balanceAtHeight(miner.address, lastCheckHeight) shouldBe
        startMinerBalance + dAppTacOutgo + blockReward * (lastCheckHeight - firstCheckHeight)
    }
  ...
  "Restrictions" - {
    ...
    "SponsorFee is available for assets issued via transaction" in {
      ...
      miner.transfer(sender.keyPair, dApp.toAddress.toString, initialTacBalance, minFee, waitForTx = true)
    ...
    "Negative fee is not available" in {
      val dApp        = miner.createKeyPair()
      val dAppAddress = dApp.toAddress.toString
      miner.transfer(sender.keyPair, dAppAddress, initialTacBalance, minFee, waitForTx = true)
      val assetId = miner.issue(dApp, waitForTx = true).id
    ...
    "SponsorFee is available only for assets issuing from current address" in {
      ...
      miner.transfer(sender.keyPair, issuer.toAddress.toString, initialTacBalance, minFee, waitForTx = true)
  ...
  private def createDApp(script: String, address: KeyPair = miner.createKeyPair()): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.toAddress.toString, initialTacBalance, minFee, waitForTx = true)
