...
import com.tacplatform.api.http.ApiError.{ScriptExecutionError, TransactionNotAllowedByAssetScript}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.it.api.SyncHttpApi._
import com.tacplatform.it.sync.{issueFee, minFee, smartFee, smartMinFee}
...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("invoke by smart account requires just 1 extra fee") {
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 TAC"
    )
  }

  test("max fee is 0.053 Tac (0.005 + extraFee(1 smart caller + 1 payment + 10 transfers))") {
    ...
    assertApiError(
      ...
      AssertiveApiError(
        ...
        "Error while executing account-script: Fee in TAC for InvokeScriptTransaction (5299999 in TAC) with 12 total scripts invoked does not exceed minimal value of 5300000 TAC."
      )
    )
  ...
  test("can't invoke with small fee for both smart assets") {
    ...
    assertBadRequestAndMessage(
      sender
        ...
        ._1
        .id,
      "does not exceed minimal value of 900000 TAC"
    )
  }
  ...
  test("can't invoke with small fee for one of smart assets") {
    ...
    assertApiError(
      ...
      AssertiveApiError(
        ScriptExecutionError.Id,
        "Error while executing account-script: Fee in TAC for InvokeScriptTransaction (900000 in TAC) with 2 total scripts invoked does not exceed minimal value of 1300000 TAC."
      )
    )
  }
  ...
  protected override def beforeAll(): Unit = {
    ...
    withClue("send tac to dApp and caller accounts") {
      val dAppTransferId        = sender.transfer(sender.keyPair, dAppAddress, 5.tac, minFee).id
      val callerTransferId      = sender.transfer(sender.keyPair, callerAddress, 5.tac, minFee).id
      val smartCallerTransferId = sender.transfer(sender.keyPair, smartCallerAddress, 5.tac, minFee).id
