package com.tacplatform.it.api
...
import com.tacplatform.account.{AddressOrAlias, AddressScheme, KeyPair}
import com.tacplatform.api.http.DebugMessage._
import com.tacplatform.api.http.RewardApiRoute.RewardStatus
import com.tacplatform.api.http.requests.{IssueRequest, TransferRequest}
import com.tacplatform.api.http.{ConnectReq, DebugMessage, RollbackParams, `X-Api-Key`}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, Base64, EitherExt2}
import com.tacplatform.features.api.ActivationStatus
import com.tacplatform.it.Node
import com.tacplatform.it.util.GlobalTimer.{instance => timer}
import com.tacplatform.it.util._
import com.tacplatform.lang.script.ScriptReader
...
import com.tacplatform.state.DataEntry.Format
import com.tacplatform.state.{AssetDistribution, AssetDistributionPage, DataEntry, EmptyDataEntry, LeaseBalance, Portfolio}
import com.tacplatform.transaction.Asset.{IssuedAsset, Tac}
import com.tacplatform.transaction.assets._
import com.tacplatform.transaction.assets.exchange.{Order, ExchangeTransaction => ExchangeTx}
...
import com.tacplatform.transaction.transfer.MassTransferTransaction.{ParsedTransfer, Transfer}
import com.tacplatform.transaction.transfer._
import com.tacplatform.transaction.{Asset, CreateAliasTransaction, DataTransaction, Proofs, TxVersion}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(val n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/tac/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def invokeScript(
      ...
      signedTraceBroadcast(
        InvokeScriptTransaction(
          ...
          payment,
          fee,
          feeAssetId.map(aid => IssuedAsset(ByteStr.decodeBase58(aid).get)).getOrElse(Asset.Tac),
          System.currentTimeMillis(),
          Proofs.empty,
          AddressScheme.current.chainId
    ...
    def updateAssetInfo(
      ...
      val tx = UpdateAssetInfoTransaction(
        ...
        fee,
        if (feeAssetId.isDefined) IssuedAsset(ByteStr(Base58.decode(feeAssetId.get))) else Tac,
        Proofs.empty,
        AddressScheme.current.chainId
      ).signWith(sender.privateKey)
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateTac/$height").as[Map[String, Long]]
