...
import com.tacplatform.generator.utils.Implicits._
...
import com.tacplatform.utils.LoggerFacade
...
object Gen {
  ...
  def transfers(senderGen: Iterator[KeyPair], recipientGen: Iterator[Address], feeGen: Iterator[Long]): Iterator[Transaction] = {
    ...
    senderGen
      ...
      .map {
        case (((src, dst), fee), i) =>
          TransferTransaction.selfSigned(2.toByte, src, dst, Tac, fee, Tac, fee, ByteStr.empty, now + i)
      }
    ...
    senderGen
      ...
      .map {
        case ((sender, count), i) =>
          ...
          MassTransferTransaction.selfSigned(1.toByte, sender, Tac, transfers, fee, now + i, ByteStr.empty)
      }
