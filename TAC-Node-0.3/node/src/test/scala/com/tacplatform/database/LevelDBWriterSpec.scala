...
import com.tacplatform.settings.{GenesisSettings, TestFunctionalitySettings, TestSettings, TacSettings, loadConfig}
import com.tacplatform.state.diffs.ENOUGH_AMT
import com.tacplatform.state.utils._
import com.tacplatform.state.{BlockchainUpdaterImpl, Height, TransactionId, TxNum}
...
import com.tacplatform.{EitherMatchers, RequestGen, TransactionGen, WithDB, database}
...
class LevelDBWriterSpec
  ...
  "tacAmount" - {
    "counts genesis" in {
      ...
      leveldb.tacAmount(1) shouldBe BigInt(1234)
    }
  }
  ...
  def baseTest(gen: Time => Gen[(KeyPair, Seq[Block])])(f: (LevelDBWriter, KeyPair) => Unit): Unit = {
    ...
    val settings0     = TacSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  def createTransfer(master: KeyPair, recipient: Address, ts: Long): TransferTransaction =
    TransferTransaction
      .selfSigned(1.toByte, master, recipient, Tac, ENOUGH_AMT / 10, Tac, 1000000, ByteStr.empty, ts)
      .explicitGet()
