...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BYTESTR, CONST_LONG, FUNCTION_CALL}
import com.tacplatform.lang.v1.compiler.TestCompiler
import com.tacplatform.network.TransactionPublisher
import com.tacplatform.state.{BinaryDataEntry, Blockchain, Diff}
import com.tacplatform.state.reader.LeaseDetails
import com.tacplatform.transaction.{Asset, TxHelpers, TxVersion}
...
class LeaseRouteSpec
  ...
  private def leaseCancelTransaction(sender: KeyPair, leaseId: ByteStr) =
    LeaseCancelTransaction.selfSigned(TxVersion.V3, sender, leaseId, 0.001.tac, ntpTime.getTimestamp()).explicitGet()
        ...
        .selfSigned(
          ...
          Some(
            FUNCTION_CALL(
              ...
              List(CONST_BYTESTR(ByteStr(recipient.toAddress.bytes)).explicitGet(), CONST_LONG(10_000.tac))
            )
          ),
          Seq.empty,
          0.005.tac,
      ...
      case (sender, genesis, setScript, invoke, recipient) =>
        withRoute { (d, r) =>
          ...
          val leaseId = d.blockchain
            ...
            .get
          val expectedDetails = Seq(leaseId -> LeaseDetails(setScript.sender, recipient, 10_000.tac, LeaseDetails.Status.Active, invoke.id(), 1))
          // check liquid block
    ...
    "created by nested invocations" in forAll(nestedInvocation) {
      case ((proxy, target, recipient), genesisTransactions) =>
        withRoute { (d, r) =>
          ...
          val expectedDetails = Seq(leaseId -> LeaseDetails(target.publicKey, recipient, 10_000.tac, LeaseDetails.Status.Active, ist.id(), 1))
          // check liquid block
