...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_LONG, FUNCTION_CALL}
...
import com.tacplatform.transaction.{Asset, CreateAliasTransaction, DataTransaction, Proofs, Transaction, TxVersion, VersionedTransaction}
import com.tacplatform.utx.UtxPool
import com.tacplatform.{TestWallet, TransactionGen}
...
class ProtoVersionTransactionsSpec
  ...
  private val MinFee: Long            = (0.001 * Constants.UnitsInTac).toLong
  ...
  private val SetScriptFee: Long      = (0.01 * Constants.UnitsInTac).toLong
  private val SetAssetScriptFee: Long = Constants.UnitsInTac
  ...
  "Proto transactions should be able to broadcast " - {
    ...
    "InvokeScriptTransaction" in {
      ...
      val invokeScriptTxUnsigned = InvokeScriptTransaction
        .create(
          ...
          Seq(InvokeScriptTransaction.Payment(100L, Asset.Tac)),
          InvokeScriptTxFee,
    ...
    "TransferTransaction" in {
      ...
      val transferTxUnsigned =
        TransferTransaction
          .create(TxVersion.V3, account.publicKey, recipient, asset, 100L, Asset.Tac, MinFee, ByteStr(attachment), Now, Proofs.empty)
          .explicitGet()
    ...
    "MassTransferTransaction" in {
      ...
      val massTransferTxUnsigned =
        MassTransferTransaction
          .create(TxVersion.V2, account.publicKey, Asset.Tac, transfers, MassTransferTxFee, Now, ByteStr(attachment), Proofs.empty)
          .explicitGet()
