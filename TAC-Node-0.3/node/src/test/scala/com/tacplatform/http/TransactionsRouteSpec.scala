...
import com.tacplatform.account.{AddressScheme, KeyPair, PublicKey}
...
import com.tacplatform.api.http.TransactionsApiRoute
import com.tacplatform.block.Block
import com.tacplatform.block.Block.TransactionProof
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, _}
...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_LONG, FUNCTION_CALL, TRUE}
import com.tacplatform.lang.v1.estimator.v3.ScriptEstimatorV3
import com.tacplatform.lang.v1.traits.domain.{Lease, LeaseCancel, Recipient}
...
import com.tacplatform.state.diffs.{ENOUGH_AMT, FeeValidation}
import com.tacplatform.state.reader.LeaseDetails
import com.tacplatform.state.{AccountScriptInfo, Blockchain, Height, InvokeScriptResult}
...
import com.tacplatform.transaction.smart.script.trace.{AccountVerifierTrace, TracedResult}
import com.tacplatform.transaction.transfer.{MassTransferTransaction, TransferTransaction}
import com.tacplatform.transaction.{Asset, Proofs, Transaction, TxHelpers, TxVersion}
import com.tacplatform.{BlockGen, BlockchainStubHelpers, NoShrink, TestTime, TestValues, TestWallet, TransactionGen}
  ...
  routePath("/calculateFee") - {
    "transfer with Tac fee" - {
      ...
      "TransferTransaction" in forAll(transferTxScenario) {
        case (_, transferTx) =>
          (addressTransactions.calculateFee _).expects(*).returning(Right((Asset.Tac, 100000L, 0L))).once()
      ...
      "MassTransferTransaction" in forAll(massTransferTxScenario) {
        case (_, transferTx) =>
          (addressTransactions.calculateFee _).expects(*).returning(Right((Asset.Tac, 200000L, 0L))).once()
