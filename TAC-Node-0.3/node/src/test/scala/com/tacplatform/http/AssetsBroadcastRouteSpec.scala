...
import com.tacplatform.api.http.assets._
import com.tacplatform.api.http.requests.{SignedTransferV1Request, SignedTransferV2Request}
...
import com.tacplatform.transaction.{Asset, Proofs, Transaction}
  ...
  "returns appropriate error code when validation fails for" - {
    ...
    "transfer transaction" in {
      forAll(broadcastTransferReq) { tr =>
        ...
        forAll(nonPositiveLong) { q =>
          posting(tr.copy(amount = q)) should produce(NonPositiveAmount(s"$q of ${tr.assetId.getOrElse("tac")}"))
        }
        forAll(invalidBase58) { pk =>
