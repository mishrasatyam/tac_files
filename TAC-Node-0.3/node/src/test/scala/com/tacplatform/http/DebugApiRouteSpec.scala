...
import com.tacplatform.api.http.DebugApiRoute
...
import com.tacplatform.lang.v1.traits.domain.{Issue, Lease, LeaseCancel, Recipient}
import com.tacplatform.network.PeerDatabase
...
import com.tacplatform.state.{AccountScriptInfo, AssetDescription, AssetScriptInfo, Blockchain, Height, InvokeScriptResult, NG, StateHash}
...
import com.tacplatform.transaction.{TxHelpers, TxVersion}
import com.tacplatform.{BlockchainStubHelpers, NTPTime, TestValues, TestWallet, TransactionGen}
...
class DebugApiRouteSpec
  ...
  val tacSettings = TacSettings.default()
  val configObject  = tacSettings.config.root()
  ...
  routePath("/validate") - {
    ...
    "takes the priority pool into account" in withDomain(DomainPresets.NG) { d =>
      ...
      d.appendBlock(TxHelpers.transfer(to = TxHelpers.secondAddress, amount = 1.tac + TestValues.fee))

      val route = debugApiRoute.copy(priorityPoolBlockchain = () => d.blockchain).route
      val tx    = TxHelpers.transfer(TxHelpers.secondSigner, TestValues.address, 1.tac)
    ...
    "valid tx" in {
      ...
      val tx = TxHelpers.transfer(TxHelpers.defaultSigner, TestValues.address, 1.tac)
      validatePost(tx) ~> route ~> check {
        val json = Json.parse(responseAs[String])
        (json \ "valid").as[Boolean] shouldBe true
        (json \ "validationTime").as[Int] shouldBe 1000 +- 1000
    ...
    "txs with empty and small verifier" in {
      val blockchain = createBlockchainStub { blockchain =>
        ...
        (() => blockchain.settings).when().returns(TacSettings.default().blockchainSettings.copy(functionalitySettings = settings))
      ...
      val tx = TxHelpers.transfer(TxHelpers.defaultSigner, TxHelpers.secondSigner.toAddress, 1.tac, fee = transferFee, version = TxVersion.V2)
      validatePost(tx) ~> route ~> check {
        val json = Json.parse(responseAs[String])
        (json \ "valid").as[Boolean] shouldBe true
      }

      val tx2 = TxHelpers.transfer(TxHelpers.secondSigner, TestValues.address, 1.tac, fee = transferFee, version = TxVersion.V2)
      ...
      val tx3 = TxHelpers.transfer(TxHelpers.signer(3), TestValues.address, 1.tac, fee = transferFee, version = TxVersion.V2)
