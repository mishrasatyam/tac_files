...
import com.tacplatform.lang.v1.testing.{ScriptGen, TypedScriptGen}
import com.tacplatform.lang.v1.{ContractLimits, FunctionHeader}
import com.tacplatform.settings.{Constants, FunctionalitySettings}
...
import com.tacplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  protected def tac(n: Float): Long = (n * 100000000L).toLong
  ...
  val tacAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, tacAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen: Gen[AssetPair] = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(Tac, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      ...
      Gen.oneOf(Gen.const(None), a2bytesGen).map { a2 =>
        ...
        val asset2 = a2.fold[Asset](Tac)(arr => IssuedAsset(ByteStr(arr)))
  ...
  def issueAndSetAssetScriptGen(sender: KeyPair): Gen[(IssueTransaction, SetAssetScriptTransaction)] =
    for {
      ...
      setAssetScript = SetAssetScriptTransaction
        .selfSigned(1.toByte, sender, IssuedAsset(issue.id()), Some(script2), 1 * Constants.UnitsInTac + ScriptExtraFee, timestamp)
        .explicitGet()
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransaction] =
    ...
    } yield TransferTransaction.selfSigned(1.toByte, sender, recipient, Tac, amount, Tac, feeAmount, attachment, timestamp).explicitGet()

  def transferGeneratorPV2(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransaction] =
    for {
      amount                                    <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield TransferTransaction.selfSigned(2.toByte, sender, recipient, Tac, amount, Tac, feeAmount, attachment, timestamp).explicitGet()
  ...
  def tacTransferGeneratorP(sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransaction] =
    transferGeneratorP(sender, recipient, Tac, Tac)

  def tacTransferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransaction] =
    transferGeneratorP(timestamp, sender, recipient, Tac, Tac)
  ...
  def createTacTransfer(
    ...
    TransferTransaction.selfSigned(1.toByte, sender, recipient, Tac, amount, Tac, fee, ByteStr.empty, timestamp)
  ...
  def versionedTransferGenP(sender: PublicKey, recipient: Address, proofs: Proofs): Gen[TransferTransaction] =
    (for {
      ...
      fee       <- smallFeeGen
      timestamp <- timestampGen
    } yield TransferTransaction(2.toByte, sender, recipient, Tac, amt, Tac, fee, ByteStr.empty, timestamp, proofs, recipient.chainId))
      .label("VersionedTransferTransactionP")

  val transferWithTacFeeGen: Gen[TransferTransaction] = for {
    (assetId, sender, recipient, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransaction.selfSigned(1.toByte, sender, recipient, assetId, amount, Tac, feeAmount, attachment, timestamp).explicitGet()

  val selfTransferWithTacFeeGen: Gen[TransferTransaction] = for {
    (assetId, sender, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransaction.selfSigned(1.toByte, sender, sender.toAddress, assetId, amount, Tac, feeAmount, attachment, timestamp).explicitGet()
  ...
  val updateAssetInfoTxGen: Gen[UpdateAssetInfoTransaction] =
    for {
      ...
      tx = UpdateAssetInfoTransaction
        .selfSigned(1.toByte, account, ByteStr(assetId), new String(assetName), new String(description), timestamp, fee, Tac)
        .explicitGet()
    } yield tx
  ...
  def sponsorFeeCancelSponsorFeeGen(sender: KeyPair, reducedFee: Boolean = true): Gen[(IssueTransaction, SponsorFeeTransaction, SponsorFeeTransaction, SponsorFeeTransaction)] =
    for {
      ...
      minFee1 <- smallFeeGen
      assetId = issue.asset
      fee = (if (reducedFee) 0.001 * Constants.UnitsInTac else 1 * Constants.UnitsInTac.toDouble).toLong
    } yield (
      issue,
  ...
  def invokeScriptGen(paymentsGen: Gen[Seq[Payment]]): Gen[InvokeScriptTransaction] =
    for {
      ...
      .selfSigned(1.toByte, sender, dappAddress.toAddress, Some(fc), payments, fee, Tac, timestamp)
      .explicitGet()

  val paymentListGen: Gen[Seq[Payment]] =
    for {
      tacPayment <- Gen.option(positiveLongGen.map(Payment(_, Tac)))
      assetPayment = for {
    ...
    } yield assetPayments ++ tacPayment
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId  <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, buyerAnotherAsset, Tac)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, sellerAnotherAsset, Tac)
    r <- Gen.oneOf(
  ...
  def exchangeV2GeneratorP(
      ...
      buyMatcherFeeAssetId: Asset = Tac,
      sellMatcherFeeAssetId: Asset = Tac,
      fixedMatcher: Option[KeyPair] = None
  ): Gen[ExchangeTransaction] = {
