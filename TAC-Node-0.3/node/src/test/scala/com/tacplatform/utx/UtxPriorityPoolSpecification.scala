...
import com.tacplatform.mining.{MultiDimensionalMiningConstraint, OneDimensionalMiningConstraint, TxEstimators}
import com.tacplatform.settings.TacSettings
import com.tacplatform.state.diffs.ENOUGH_AMT
import com.tacplatform.state.{AccountScriptInfo, Blockchain, Diff, LeaseBalance, Portfolio}
import com.tacplatform.transaction.Asset.Tac
import com.tacplatform.transaction.transfer.TransferTransaction
import com.tacplatform.transaction.{Transaction, TxHelpers, TxVersion}
import com.tacplatform.utils.Time
import com.tacplatform.utx.UtxPool.PackStrategy
import com.tacplatform.{BlocksTransactionsHelpers, EitherMatchers, NoShrink, TestValues, TransactionGen}
...
class UtxPriorityPoolSpecification
  ...
  "priority pool" - {
      ...
      def setPriorityTxs(txs: Seq[Transaction]): Unit = {
        val asDiffs = txs.map {
          case tt: TransferTransaction =>
            ...
            ).view.mapValues(Portfolio.tac).toMap
    ...
    def createState(scripted: Address, settings: TacSettings = TacSettings.default(), setBalance: Boolean = true): Blockchain = {
  ...
  private def transferV2(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      .selfSigned(TxVersion.V2, sender, recipient.toAddress, Tac, amount, Tac, fee, ByteStr.empty, time.getTimestamp())
      .explicitGet())
