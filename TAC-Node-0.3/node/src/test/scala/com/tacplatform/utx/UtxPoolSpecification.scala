...
import com.tacplatform.database.{LevelDBWriter, TestStorageFactory, openDB}
...
import com.tacplatform.history.{DefaultTacSettings, randomSig, settingsWithFeatures}
...
import com.tacplatform.lang.v1.compiler.{CompilerContext, ExpressionCompiler}
...
import com.tacplatform.transaction.TxValidationError.{GenericError, SenderIsBlacklisted}
...
import com.tacplatform.transaction.{Asset, Transaction, _}
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(genAccounts: Map[Address, Long]) = {
    ...
    val origSettings    = TacSettings.default()
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      .selfSigned(1.toByte, sender, recipient.toAddress, Tac, amount, Tac, fee, ByteStr.empty, time.getTimestamp())
      .explicitGet())
  ...
  private def massTransferWithRecipients(sender: KeyPair, recipients: List[PublicKey], maxAmount: Long, time: Time) = {
    ...
    val txs = for { fee <- chooseNum(minFee, amount) } yield MassTransferTransaction
      .selfSigned(1.toByte, sender, Tac, transfers, fee, time.getTimestamp(), ByteStr.empty)
      .explicitGet()
  ...
  private def invokeScript(sender: KeyPair, dApp: Address, time: Time) =
    Gen.choose(500000L, 600000L).map { fee =>
      InvokeScriptTransaction.selfSigned(TxVersion.V1, sender, dApp, None, Seq.empty, fee, Tac, time.getTimestamp()).explicitGet()
    }
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createTacTransfer(sender, recipient.toAddress, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(1.toByte, sender, recipient.toAddress, Tac, tac(1), Tac, feeAmount, ByteStr.empty, ts).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(2.toByte, sender, recipient.toAddress, Tac, tac(1), Tac, feeAmount, ByteStr.empty, ts).explicitGet()
  }
  ...
  "UTX Pool" - {
    ...
    "correctly process constraints in packUnconfirmed" in {
      withDomain(tacplatform.history.TransfersV2ActivatedAt0TacSettings) { d =>
    ...
    "smart accounts" - {
      ...
      "dApp to dApp call chain in force validate mode" in withDomain(
        ...
        val utx    = new UtxPoolImpl(ntpTime, d.blockchainUpdater, PublishSubject(), DefaultTacSettings.utxSettings)
      ...
      "doesnt validate transactions which are removed" in {
        ...
        forAll(gen) {
          case (tx1, rest) =>
            ...
            (() => blockchain.settings).when().returning(TacSettings.default().blockchainSettings)
            (() => blockchain.height).when().returning(1)
            (() => blockchain.activatedFeatures).when().returning(Map.empty)

            val utx =
              new UtxPoolImpl(ntpTime, blockchain, ignoreSpendableBalanceChanged, TacSettings.default().utxSettings)
      ...
      "fires events correctly" in {
        val preconditions = for {
          ...
          validTransfer = TransferTransaction
            .selfSigned(TxVersion.V1, richAcc, secondAcc.toAddress, Tac, 1L, Tac, fee, ByteStr.empty, ts)
            .explicitGet()
          invalidTransfer = TransferTransaction
            .selfSigned(TxVersion.V1, secondAcc, richAcc.toAddress, Tac, 2L, Tac, fee, ByteStr.empty, ts)
            .explicitGet()
        ...
        forAll(preconditions) {
          case (genesis, validTransfer, invalidTransfer) =>
            withDomain() { d =>
              ...
              val utxPool =
                new UtxPoolImpl(time, d.blockchainUpdater, ignoreSpendableBalanceChanged, TacSettings.default().utxSettings, events += _)
