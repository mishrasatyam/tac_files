...
import com.tacplatform.database.{Keys, TestStorageFactory}
...
import com.tacplatform.state.{Blockchain, BlockchainUpdaterImpl, NG}
import com.tacplatform.transaction.Asset.Tac
import com.tacplatform.transaction.transfer.TransferTransaction
import com.tacplatform.transaction.{BlockchainUpdater, GenesisTransaction, Transaction}
...
class MiningWithRewardSuite extends AsyncFlatSpec with Matchers with WithDB with TransactionGen with DBCacheSettings {
  ...
  it should "generate valid blocks with transactions of version 4" in {
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = TransferTransaction
          .selfSigned(2.toByte, account, recipient1, Tac, 10 * Constants.UnitsInTac, Tac, 400000, ByteStr.empty, ts)
          .explicitGet()
        val tx2 = TransferTransaction
          .selfSigned(2.toByte, account, recipient2, Tac, 5 * Constants.UnitsInTac, Tac, 400000, ByteStr.empty, ts)
          .explicitGet()
    ...
    withEnv(bps, txs) {
      case Env(_, account, miner, blockchain) =>
        val generateBlock = generateBlockTask(miner)(account)
        val oldBalance    = blockchain.balance(account.toAddress)
        val newBalance    = oldBalance + settings.blockchainSettings.rewardsSettings.initial - 10 * Constants.UnitsInTac
  ...
  private def withEnv(bps: Seq[BlockProducer], txs: Seq[TransactionProducer] = Seq(), settings: TacSettings = MiningWithRewardSuite.settings)(
  ...
  private def resources(settings: TacSettings): Resource[Task, (BlockchainUpdaterImpl, DB)] =
    Resource.make {
...
object MiningWithRewardSuite {
  ...
  val settings: TacSettings = {
    val commonSettings: TacSettings = TacSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  def settingsWithFeatures(features: BlockchainFeature*): TacSettings = {
