...
import com.tacplatform.block.serialization.{BlockHeaderSerializer, BlockSerializer}
import com.tacplatform.block.validation.Validators
...
import com.tacplatform.history.chainBaseAndMicro
import com.tacplatform.lagonaki.mocks.TestBlock
import com.tacplatform.lang.ValidationError
import com.tacplatform.protobuf.block.PBBlocks
import com.tacplatform.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings, WalletSettings, TacSettings}
import com.tacplatform.state.appender.BlockAppender
import com.tacplatform.state.{Blockchain, BlockchainUpdaterImpl, NG, diffs}
import com.tacplatform.transaction.Asset.Tac
import com.tacplatform.transaction.transfer.TransferTransaction
import com.tacplatform.transaction.{BlockchainUpdater, GenesisTransaction, Transaction, TxVersion}
import com.tacplatform.utils.Time
import com.tacplatform.utx.UtxPoolImpl
import com.tacplatform.wallet.Wallet
import com.tacplatform.{BlocksTransactionsHelpers, EitherMatchers, NoShrink, TestTime, TransactionGen, crypto, protobuf}
...
class BlockV5Test
  ...
  private def createTx(sender: KeyPair, recipient: AddressOrAlias): Transaction =
    TransferTransaction
      .selfSigned(TxVersion.V1, sender, recipient, Tac, 10 * Constants.UnitsInTac, Tac, 100000, ByteStr.empty, ntpTime.getTimestamp())
      .explicitGet()
  ...
  "blockId" should "be header hash" in {
    ...
    forAll(preconditions) {
      case (acc, genesis) =>
        ...
        withDomain(TacSettings.default().copy(blockchainSettings = TacSettings.default().blockchainSettings.copy(functionalitySettings = fs))) {
          d =>
  ...
  private def genesis: Gen[(KeyPair, KeyPair, Block)] =
    for {
      ...
      genesisBlock = TestBlock.create(
        ...
        txs = Seq(
          GenesisTransaction.create(miner1.toAddress, Constants.TotalTac / 2 * Constants.UnitsInTac, ntpNow).explicitGet(),
          GenesisTransaction.create(miner2.toAddress, Constants.TotalTac / 2 * Constants.UnitsInTac, ntpNow).explicitGet()
        ),
        version = Block.GenesisBlockVersion
      )
    } yield (miner1, miner2, genesisBlock)

  private def withBlockchain(disabledFeatures: AtomicReference[Set[Short]], time: Time = ntpTime, settings: TacSettings = testSettings)(
  ...
  private def withMiner(blockchain: Blockchain with BlockchainUpdater with NG, time: Time, settings: TacSettings = testSettings)(
...
object BlockV5Test {
  ...
  private val defaultSettings = TacSettings.fromRootConfig(ConfigFactory.load())
