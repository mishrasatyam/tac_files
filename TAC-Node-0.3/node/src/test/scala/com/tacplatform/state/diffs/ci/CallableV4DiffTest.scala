...
import com.tacplatform.lang.directives.values.{Asset, V4}
...
import com.tacplatform.state.diffs.{ENOUGH_AMT, _}
import com.tacplatform.state.{EmptyDataEntry, SponsorshipValue}
...
import com.tacplatform.transaction.{GenesisTransaction, Transaction, TxVersion}
    ...
    forAll(multiActionPreconditions(feeMultiplier = 5, withScriptError = false)) {
      case (genesis, setScript, invoke, issue, _, _, _, _, _) =>
        assertDiffEi(
          ...
          TestBlock.create(Seq(invoke)),
          features
        )(_ should produce(s" with 6 total scripts invoked does not exceed minimal value of $minimalFee TAC"))
    }
  }
    ...
    val deleteEntryPreconditions: Gen[(List[GenesisTransaction], SetScriptTransaction, InvokeScriptTransaction, KeyPair)] =
      for {
        ...
        for {
          ...
          ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, Nil, fee, Tac, ts + 3)
  ...
  private def multiActionPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(TxVersion.V1, invoker, master.toAddress, None, Nil, fee, Tac, ts + 3)
