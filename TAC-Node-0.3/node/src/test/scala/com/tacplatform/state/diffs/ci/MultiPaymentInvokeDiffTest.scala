...
class MultiPaymentInvokeDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithState with Inside {
  ...
  property("multi payment with verifier and transfer set") {
    ...
    forAll(
      paymentPreconditions(
        dApp(V4, transferPaymentAmount = tacTransfer, _),
      ...
      case (genesis, setVerifier, setDApp, ci, issues, dAppAcc, invoker, fee) =>
        assertDiffAndState(
          ...
          case (diff, blockchain) =>
            ...
            diff.portfolios(dAppAcc.toAddress).balance shouldBe -tacTransfer
            diff.portfolios(invoker.toAddress).balance shouldBe tacTransfer - fee
        }
    }
  }
    ...
    forAll(
      ...
      case (genesis, setVerifier, setDApp, ci, issues, _, _, _) =>
        assertDiffEi(
          ...
          val expectedFee = (0.005 + 0.004 + 0.004 * (ContractLimits.MaxAttachedPaymentAmount - 1)) * Constants.UnitsInTac
          _ should produce(
            s"Fee in TAC for InvokeScriptTransaction (${ci.fee} in TAC) " +
              s"with ${ContractLimits.MaxAttachedPaymentAmount} total scripts invoked " +
              s"does not exceed minimal value of ${expectedFee.toLong} TAC"
          )
        }
    }
  }
  ...
  property("single payment V3 scripts with activated multi payment") {
    ...
    forAll(
      paymentPreconditions(
        dApp(V3, transferPaymentAmount = tacTransfer, _),
  ...
  private def paymentPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, payments, fee, Tac, ts + 3)
