...
import com.tacplatform.lang.directives.values.{StdLibVersion, V3, V5}
...
class InvokePaymentsAvailabilityTest
  ...
  private def scenario(version: StdLibVersion, syncCall: Boolean) =
    for {
      ...
      invokeTx = InvokeScriptTransaction.selfSigned(TxVersion.V3, invoker, dApp, None, payments, fee, Tac, ts).explicitGet()
