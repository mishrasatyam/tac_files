...
import com.tacplatform.state.{LeaseBalance, Portfolio}
...
import com.tacplatform.transaction.{Authorized, GenesisTransaction, Transaction}
  ...
  private def leasePreconditions(
    ...
    for {
      ...
      for {
        ...
        invoke   <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, dAppAcc.toAddress, None, Nil, fee, Tac, ts)
    ...
    forAll(leasePreconditions()) {
      case (preparingTxs, invoke, leaseAmount, dAppAcc, invoker, _, _) =>
        withDomain(domainSettingsWithFS(v5Features)) { d =>
          ...
          val invokerPortfolio = d.blockchain.tacPortfolio(invoker)
          ...
          val dAppPortfolio = d.blockchain.tacPortfolio(dAppAcc)
    ...
    forAll(leasePreconditions(customAmount = Some(0))) {
      case (preparingTxs, invoke, _, _, _, _, _) =>
        assertDiffAndState(
          ...
          case (diff, _) =>
            diff.errorMessage(invoke.id.value()).get.text shouldBe "NonPositiveAmount(0,tac)"
        }
    }
  }
    ...
    forAll(leasePreconditions(customAmount = Some(-100))) {
      case (preparingTxs, invoke, _, _, _, _, _) =>
        assertDiffAndState(
          ...
          case (diff, _) =>
            diff.errorMessage(invoke.id.value()).get.text shouldBe "NonPositiveAmount(-100,tac)"
        }
    }
  }
    ...
    forAll(leasePreconditions(customSetScriptFee = Some(setScriptFee), customAmount = Some(dAppBalance))) {
      case (preparingTxs, invoke, _, dAppAcc, _, _, _) =>
        assertDiffAndState(
          ...
          case (_, blockchain) =>
            blockchain.tacPortfolio(dAppAcc).effectiveBalance shouldBe 0
        }
    }
  }
    ...
    forAll(leasePreconditions(customDApp = Some(leaseWithLeaseCancelDApp(recipient.toRide, amount)))) {
      case (preparingTxs, invoke, _, dAppAcc, _, _, _) =>
        withDomain(domainSettingsWithFS(v5Features)) { d =>
          ...
          val recipientPortfolio = d.blockchain.tacPortfolio(recipient)
