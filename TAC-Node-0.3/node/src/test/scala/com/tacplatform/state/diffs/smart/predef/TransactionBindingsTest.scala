package com.tacplatform.state.diffs.smart.predef

import com.tacplatform.account.{Address, Alias}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.state.diffs.ProduceError._
...
import com.tacplatform.lang.Testing.evaluated
...
import com.tacplatform.lang.v1.{ContractLimits, compiler}
import com.tacplatform.state._
import com.tacplatform.transaction.Asset.Tac
import com.tacplatform.transaction.assets.exchange.{Order, OrderType}
import com.tacplatform.transaction.smart.BlockchainContext.In
import com.tacplatform.transaction.smart.{TacEnvironment, buildThisValue}
import com.tacplatform.transaction.{DataTransaction, Proofs, ProvenTransaction, TxVersion, VersionedTransaction}
...
class TransactionBindingsTest
    ...
    forAll(invokeScriptGen(paymentOptionGen)) { t =>
      ...
      val script =
        s"""
           ...
           |     then if (${t.payments.headOption.exists(_.assetId != Tac)})
  ...
  property("MassTransferTransaction binding") {
    forAll(massTransferGen) { t =>
      ...
      val script = s"""
                      ...
                      |    let assetId = if (${t.assetId != Tac}) then extract(t.assetId) == base58'${t.assetId.maybeBase58Repr
                        .getOrElse("")}'
                      |      else isDefined(t.assetId) == false
    ...
    forAll(exchangeTransactionGen) { t =>
      def pg(ord: Order) = {
        ...
        val script = s"""
                        ...
                        |   let ${oType}AssetPairAmount = if (${ord.assetPair.amountAsset != Tac}) then extract(t.${oType}Order.assetPair.amountAsset) == base58'${ord.assetPair.amountAsset.maybeBase58Repr
                          .getOrElse("")}'
                        |   else isDefined(t.${oType}Order.assetPair.amountAsset) == false
                        |   let ${oType}AssetPairPrice = if (${ord.assetPair.priceAsset != Tac}) then extract(t.${oType}Order.assetPair.priceAsset) == base58'${ord.assetPair.priceAsset.maybeBase58Repr
                          .getOrElse("")}'
                        |   else isDefined(t.${oType}Order.assetPair.priceAsset) == false
                        |   let ${oType}MatcherFeeAssetId = if (${ord.matcherFeeAssetId != Tac}) then extract(t.${oType}Order.matcherFeeAssetId) == base58'${ord.matcherFeeAssetId.maybeBase58Repr
                          .getOrElse("")}'
  ...
  property("Order binding") {
    forAll(orderGen) { t =>
      val s = s"""
                 ...
                 |   ${Range(0, 8).map(letProof(t.proofs, "t")).mkString("\n")}
                 |   let assetPairAmount = if (${t.assetPair.amountAsset != Tac}) then extract(t.assetPair.amountAsset) == base58'${t.assetPair.amountAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.amountAsset) == false
                 |   let assetPairPrice = if (${t.assetPair.priceAsset != Tac}) then extract(t.assetPair.priceAsset) == base58'${t.assetPair.priceAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.priceAsset) == false
                 |   let matcherFeeAssetId = if (${t.matcherFeeAssetId != Tac}) then extract(t.matcherFeeAssetId) == base58'${t.matcherFeeAssetId.maybeBase58Repr
                   .getOrElse("")}'
  ...
  def runForAsset(script: String): Either[String, EVALUATED] = {
    ...
    import com.tacplatform.lang.v1.CTX._
    ...
    val ctx =
      PureContext.build(V2).withEnvironment[Environment] |+|
        ...
        TacContext.build(Global, DirectiveSet(V2, Asset, Expression).explicitGet())

    val environment = new TacEnvironment(
      chainId,
      Coeval(???),
      null,
      EmptyBlockchain,
