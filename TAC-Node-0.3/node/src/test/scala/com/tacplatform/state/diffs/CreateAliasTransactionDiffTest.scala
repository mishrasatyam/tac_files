...
import com.tacplatform.state.utils.addressTransactions
import com.tacplatform.transaction.Asset.{IssuedAsset, Tac}
import com.tacplatform.transaction.assets.IssueTransaction
import com.tacplatform.transaction.{Asset, CreateAliasTransaction, GenesisTransaction, Transaction}
...
class CreateAliasTransactionDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("can create and resolve aliases preserving tac invariant") {
          ...
          case (blockDiff, _) =>
            if (transfer.sender.toAddress != aliasTx.sender.toAddress) {
              ...
              transfer.assetId match {
                ...
                case Tac                => recipientPortfolioDiff shouldBe Portfolio(transfer.amount, LeaseBalance.empty, Map.empty)
              }
            }
        }
    }
