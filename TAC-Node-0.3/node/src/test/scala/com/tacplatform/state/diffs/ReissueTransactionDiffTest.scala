...
class ReissueTransactionDiffTest
  ...
  property("Reissue transaction's fee before feature activation is 1 TAC") {
  ...
  property("Reissue transaction's fee after feature activation is 0.001 TAC") {
  ...
  private def genesis: Gen[(KeyPair, Block)] =
    for {
      ...
      block = TestBlock.create(
        ntpNow,
        Seq(GenesisTransaction.create(issuer.toAddress, Constants.TotalTac * Constants.UnitsInTac, ntpNow).explicitGet())
      )
    } yield (issuer, block)
...
object ReissueTransactionDiffTest {
  ...
  val BeforeActivationFee: Long = 1 * Constants.UnitsInTac
