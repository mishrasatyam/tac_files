...
import com.tacplatform.features.{BlockchainFeatures, EstimatorProvider}
import com.tacplatform.lagonaki.mocks.TestBlock
import com.tacplatform.lang.directives.values.{Expression, StdLibVersion, V1, V4}
...
import com.tacplatform.{BlocksTransactionsHelpers, NoShrink, TransactionGen}
...
class AssetTransactionsDiffTest
  ...
  property("Issue+Reissue+Burn do not break tac invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      assetTransfer <- transferGeneratorP(issuer, burner.toAddress, issue.asset, Tac)
      tacTransfer <- tacTransferGeneratorP(issuer, burner.toAddress)
      burn = BurnTransaction
        .selfSigned(1.toByte, burner, issue.asset, assetTransfer.amount, tacTransfer.amount, timestamp)
        .explicitGet()
    } yield (genesis, issue, assetTransfer, tacTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, tacTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, tacTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer = TransferTransaction.selfSigned(1.toByte, issuer, holder.toAddress, assetId, quantity - 1, Tac, fee, ByteStr(attachment), timestamp)
        .explicitGet()
  ...
  def genesisIssueTransferReissue(
    ...
    for {
      timestamp          <- timestampGen
      initialTacAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1L, 10L)
      genesisTx1 = GenesisTransaction.create(accountA.toAddress, initialTacAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB.toAddress, initialTacAmount, timestamp).explicitGet()
      reissuable = true
      ...
      transfer = TransferTransaction
        .selfSigned(TxVersion.V1, accountA, accountB.toAddress, assetId, issue.quantity, Tac, smallFee, ByteStr.empty,  timestamp + 2)
        .explicitGet()
