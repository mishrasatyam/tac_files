...
import com.tacplatform.lang.directives.values.{StdLibVersion, V3, V4}
  ...
  private def paymentPreconditions(dApp: Script): Gen[(List[GenesisTransaction], SetScriptTransaction, InvokeScriptTransaction, Address)] =
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, functionCall, Nil, fee, Tac, ts + 3)
