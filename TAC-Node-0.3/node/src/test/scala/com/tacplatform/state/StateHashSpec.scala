...
class StateHashSpec extends FreeSpec with Matchers {
  "state hash" - {
    ...
    stateHash.addTacBalance(address, 1000)
    val result = stateHash.result()

    def hash(bs: Array[Byte]*): ByteStr    = ByteStr(com.tacplatform.crypto.fastHash(bs.reduce(_ ++ _)))
    ...
    "total" in {
      ...
      allHashes shouldBe Seq(TacBalance, AssetBalance, DataEntry, AccountScript, AssetScript, LeaseBalance, LeaseStatus, Sponsorship, Alias)
        .map(sect)
      ...
      result.copy(hashes = result.hashes - SectionId.TacBalance).createStateHash(ByteStr.empty).totalHash shouldBe hash(
