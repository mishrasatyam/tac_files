...
import com.tacplatform.{history, NoShrink, TestTime, TransactionGen}
...
import com.tacplatform.features._
import com.tacplatform.features.BlockchainFeatures._
import com.tacplatform.history.Domain
import com.tacplatform.it.util.AddressOrAliasExt
import com.tacplatform.lagonaki.mocks.TestBlock
import com.tacplatform.lang.directives.values.V5
import com.tacplatform.lang.script.v1.ExprScript
import com.tacplatform.lang.v1.compiler.{Terms, TestCompiler}
import com.tacplatform.lang.v1.compiler.Terms.TRUE
import com.tacplatform.lang.v1.traits.domain.Lease
import com.tacplatform.lang.v1.FunctionHeader
import com.tacplatform.settings.{TestFunctionalitySettings, TacSettings}
import com.tacplatform.state.reader.LeaseDetails
import com.tacplatform.transaction.{CreateAliasTransaction, DataTransaction, GenesisTransaction, Transaction, TxVersion}
import com.tacplatform.transaction.Asset.{IssuedAsset, Tac}
import com.tacplatform.transaction.TxValidationError.AliasDoesNotExist
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    TransferTransaction.selfSigned(1.toByte, sender, recipient, Tac, amount, Tac, 1, ByteStr.empty, nextTs).explicitGet()
  ...
  private def randomOp(sender: KeyPair, recipient: Address, amount: Long, op: Int, nextTs: => Long = nextTs) = {
    ...
    op match {
      ...
      case _ => List(TransferTransaction.selfSigned(1.toByte, sender, recipient, Tac, amount, Tac, 1000, ByteStr.empty, nextTs).explicitGet())
    }
  }
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender.toAddress, com.tacplatform.state.diffs.ENOUGH_AMT))
    ...
    "tac balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
        ...
        withDomain() { d =>
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                TransferTransaction
                  .selfSigned(1.toByte, sender, recipient.toAddress, IssuedAsset(issueTransaction.id()), assetAmount, Tac, 1, ByteStr.empty, nextTs)
                  .explicitGet()
              )
            )
          )
      ...
      def appendBlock(d: Domain, invoker: KeyPair, dApp: KeyPair)(parentBlockId: ByteStr, fc: Terms.FUNCTION_CALL): ByteStr = {
        ...
        val invoke =
          InvokeScriptTransaction
            .selfSigned(2.toByte, invoker, dApp.toAddress, Some(fc), Seq.empty, fee, Tac, nextTs)
            .explicitGet()
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): TacSettings = {
      ...
      history.DefaultTacSettings.copy(blockchainSettings = history.DefaultTacSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender.toAddress, com.tacplatform.state.diffs.ENOUGH_AMT))
