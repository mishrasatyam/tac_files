...
import com.tacplatform.lang.directives.values.{V4, V5}
...
import com.tacplatform.state.InvokeScriptResult.ErrorMessage
import com.tacplatform.state.diffs.ENOUGH_AMT
import com.tacplatform.state.{Diff, InvokeScriptResult, NewTransactionInfo, Portfolio}
...
class InvokeAssetChecksTest
  ...
  private val invalidLengthAsset = IssuedAsset(ByteStr.decodeBase58("TAC").get)
  private val unexistingAsset    = IssuedAsset(ByteStr.decodeBase58("TACtacTACtacTACtacTACtac123").get)
  ...
  property("invoke asset checks") {
    ...
    val transferBase58TacDAppScenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, call, Seq(), fee, Tac, ts + 3)
          .explicitGet()
      } yield (activated, func, invokeTx, Seq(genesis1Tx, genesis2Tx, setScriptTx))

    val (activated, func, invoke, genesisTxs) = transferBase58TacDAppScenario.sample.get
    tempDb { _ =>
      ...
      val expectedResult =
        if (activated) {
          ...
          Diff.empty.copy(
            ...
            portfolios = Map(
              invoke.senderAddress -> Portfolio.tac(-invoke.fee),
              miner                -> Portfolio.tac(invoke.fee)
            ),
            scriptsComplexity = 8,
          ...
          Diff.empty.copy(
            ...
            scriptResults = Map(
              invoke.id.value() -> InvokeScriptResult(
                transfers = Seq(
                  InvokeScriptResult.Payment(invoke.senderAddress, Tac, 0),
    ...
    val preconditions =
      for {
        ...
        invokeInvalidLength = InvokeScriptTransaction
          .selfSigned(1.toByte, dAppAcc, dAppAcc.toAddress, Some(FUNCTION_CALL(User("invalidLength"), Nil)), Nil, fee, Tac, ts)
          .explicitGet()
        invokeUnexisting = InvokeScriptTransaction
          .selfSigned(1.toByte, dAppAcc, dAppAcc.toAddress, Some(FUNCTION_CALL(User("unexisting"), Nil)), Nil, fee, Tac, ts)
          .explicitGet()
