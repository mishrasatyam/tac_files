...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BOOLEAN, FUNCTION_CALL}
    ...
    val preconditions =
      for {
        ...
        invoke = InvokeScriptTransaction
          .selfSigned(1.toByte, dApp1, dApp1.toAddress, fc, Nil, fee, Tac, ts)
          .explicitGet()
  ...
  property("dApp calls dApp chain with itself at the end - prohibited") {
    val preconditions =
      for {
        ...
        invoke = InvokeScriptTransaction
          .selfSigned(1.toByte, dApp1, dApp2.toAddress, fc, Nil, fee, Tac, ts)
          .explicitGet()
