...
import com.tacplatform.state.{BooleanDataEntry, EmptyDataEntry}
...
class IsDataStorageUntouchedTest
  ...
  private val scenario =
    for {
      ...
      invokeTx     = InvokeScriptTransaction.selfSigned(TxVersion.V3, invoker, master.toAddress, None, Nil, fee, Tac, ts).explicitGet()
  ...
  property("isDataStorageUntouched in sync dApp") {
    ...
    val scenario =
      for {
        ...
        invoke   = InvokeScriptTransaction.selfSigned(1.toByte, dApp1, dApp1.toAddress, None, Nil, fee, Tac, ts).explicitGet()
