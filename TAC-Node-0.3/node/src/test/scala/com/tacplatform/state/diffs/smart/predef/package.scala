...
import com.tacplatform.transaction.smart.{BlockchainContext, buildThisValue}
...
package object predef {
  ...
  def scriptWithAllV1Functions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithV1PureFunctions(tx, t))}
       |${dropLastLine(scriptWithV1TacFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && tac else crypto""".stripMargin
  ...
  def scriptWithV1TacFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Tac context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && tacBalance(tx.sender) != 0
       |
       | let tac = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | tac""".stripMargin
