...
import com.tacplatform.transaction.{CreateAliasTransaction, GenesisTransaction, Transaction}
  ...
  private val paymentPreconditions: Gen[(List[Transaction], () => InvokeScriptTransaction, IssuedAsset, Address)] =
    for {
      ...
      for {
        ...
        invoke = () => InvokeScriptTransaction.selfSigned(1.toByte, invoker, dAppAcc.toAddress, None, Nil, fee, Tac, ts).explicitGet()
