...
import com.tacplatform.block.{Block, BlockHeader, SignedBlockHeader}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.db.{DBCacheSettings, WithState}
...
import com.tacplatform.lang.contract.DApp.{CallableAnnotation, CallableFunction}
import com.tacplatform.lang.directives.values.{DApp => DAppType, _}
...
import com.tacplatform.lang.v1.FunctionHeader.{Native, User}
...
import com.tacplatform.lang.v1.evaluator.FunctionIds.{CREATE_LIST, THROW}
import com.tacplatform.lang.v1.evaluator.ctx.impl.tac.{FieldNames, TacContext}
import com.tacplatform.lang.v1.evaluator.ctx.impl.{CryptoContext, PureContext}
import com.tacplatform.lang.v1.evaluator.{FunctionIds, ScriptResultV3}
import com.tacplatform.lang.v1.parser.{Expressions, Parser}
import com.tacplatform.lang.v1.traits.Environment
import com.tacplatform.lang.v1.{ContractLimits, FunctionHeader, compiler}
import com.tacplatform.lang.{Global, utils}
import com.tacplatform.protobuf.dapp.DAppMeta
import com.tacplatform.settings.{TestFunctionalitySettings, TestSettings}
import com.tacplatform.state._
import com.tacplatform.state.diffs.FeeValidation.FeeConstants
import com.tacplatform.state.diffs.invoke.InvokeScriptTransactionDiff
...
import com.tacplatform.transaction.smart.script.trace.{AssetVerifierTrace, InvokeScriptTrace}
...
class InvokeScriptTransactionDiffTest
  ...
  def paymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(Tac), version: StdLibVersion = V3)(func: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    } yield paymentContract(senderBinging, argBinding, func, address, amount, assets, version)

  def defaultPaymentContractGen(address: AddressOrAlias, amount: Long, assets: List[Asset] = List(Tac))(someName: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
  ...
  def preconditionsAndSetContract(
    ...
    for {
      ...
      (issueTx, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(master)
      sponsoredFee = Sponsorship.fromTac(900000L, sponsorTx.minSponsoredAssetFee.get)
      ...
      ci = InvokeScriptTransaction
        .selfSigned(
          ...
          if (sponsored) sponsorTx.asset else Tac,
          ts + 3
        )
        .explicitGet()
    ...
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffAndState(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci), Block.ProtoBlockVersion), fs) {
          case (blockDiff, newState) =>
            blockDiff.scriptsRun shouldBe 1
            newState.balance(acc.toAddress, Tac) shouldBe amount
        }
    }
  }
  ...
  property("can't make more than 10 payments") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = paymentContractGen(a.toAddress, am, List.fill(11)(Tac)) _
      r <- preconditionsAndSetContract(contractGen)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
  ...
  property("Contract payment should be positive") {
    forAll(for {
      ...
      case (acc, _, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransaction
            .selfSigned(2.toByte, master, acc.toAddress, IssuedAsset(asset.id()), asset.quantity / 10, Tac, enoughFee, ByteStr.empty, ts)
            .explicitGet()
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, Some(fc), Seq(Payment(-1, Tac)), fee, Tac, ts)
  ...
  property("can't overflow payment + fee") {
    forAll(for {
      ...
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.oneOf(Seq(invoker)),
        payment = Some(Payment(Long.MaxValue, Tac)),
        feeGen = ciFee(1)
      )
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
  ...
  property("can't overflow sum of payment in contract") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = paymentContractGen(a.toAddress, Long.MaxValue / 2 + 2, List.fill(4)(Tac)) _
      invoker <- accountGen
      ts      <- timestampGen
      r       <- preconditionsAndSetContract(contractGen, invokerGen = Gen.oneOf(Seq(invoker)), payment = Some(Payment(1, Tac)), feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          _ should produce("Attempt to transfer unavailable funds")
  ...
  property("Default function invocation should produce error if contract default function has arguments") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "default", a.toAddress, am, List(Tac))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("Default function invocation should produce error if contract does't have default function") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "undefault", a.toAddress, am, List(Tac))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("self-payment and self-transfer V3") {
    forAll(for {
      acc <- accountGen
      am  <- smallFeeGen
      contractGen = paymentContractGen(acc.toAddress, am, assets = List(Tac)) _
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.const(acc),
        masterGen = Gen.const(acc),
        payment = Some(Payment(1, Tac)),
        feeGen = ciFee(1)
      )
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
  ...
  property("self-transfer V4") {
    forAll(for {
      acc <- accountGen
      am  <- smallFeeGen
      contractGen = paymentContractGen(acc.toAddress, am, assets = List(Tac), version = V4) _
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.const(acc),
        masterGen = Gen.const(acc),
    ...
    val uniqueAssetIdScenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, Tac, ts + 3)
          .explicitGet()
  ...
  property(s"accepts failed transactions after ${BlockchainFeatures.BlockV5} activation") {
    def failInvariant(funcBinding: String, sponsorTx: SponsorFeeTransaction, issueTx: IssueTransaction): Gen[(TxAmount, Asset, DApp, List[EXPR])] = {
      val feeInTac = FeeConstants(InvokeScriptTransaction.typeId) * FeeValidation.FeeUnit
      val feeInAsset = Sponsorship.fromTac(FeeConstants(InvokeScriptTransaction.typeId) * FeeValidation.FeeUnit, sponsorTx.minSponsoredAssetFee.get)
      Gen.oneOf(
        Gen.const((feeInTac, Tac, issueContract(funcBinding), List.empty[EXPR])),           // insufficient fee
        Gen.const((feeInAsset, sponsorTx.asset, issueContract(funcBinding), List.empty[EXPR])), // insufficient fee
        Gen.const((feeInTac, Tac, throwContract(funcBinding), List.empty[EXPR])),           // DApp script execution
        ...
        } yield (fee, Tac, paymentContract, List(CONST_BYTESTR(ByteStr(arg)).explicitGet()))
      )
    }
    ...
    val failedTxScenario =
      for {
        ...
        tTx = TransferTransaction
          .selfSigned(TxVersion.V3, other, invoker.toAddress, sTx.asset, i1Tx.quantity, Tac, enoughFee, ByteStr.empty, ts)
          .explicitGet()
        funcBinding                     <- funcNameGen
    ...
    forAll(failedTxScenario) {
      case (invoke, (tacBalance, sponsoredBalance), genesisTxs) =>
        assertDiffAndState(Seq(TestBlock.create(genesisTxs)), TestBlock.create(Seq(invoke), Block.ProtoBlockVersion), fsWithV5) {
          case (diff, state) =>
            ...
            state.balance(invoke.sender.toAddress, invoke.feeAssetId) shouldBe invoke.feeAssetId.fold(tacBalance)(_ => sponsoredBalance) - invoke.fee
  ...
  property(
    ...
    val scenario =
      for {
        ...
        tTx = TransferTransaction
          .selfSigned(TxVersion.V3, other, master.toAddress, sponsoredAsset, iTx.quantity / 2, Tac, enoughFee, ByteStr.empty, ts)
          .explicitGet()
        tacFee <- ciFee(1)
        sponsoredFee = Sponsorship.fromTac(tacFee, sTx.minSponsoredAssetFee.get)
        (feeAsset, fee) <- Gen.oneOf((Tac, tacFee), (sponsoredAsset, sponsoredFee))
        funcBinding     <- funcNameGen
        arg             <- genBoundedStringBytes(1, 32)
        ...
        txs = Seq("throw", "insufficient fee", "negative amount", "overflow amount", "self payment", "max actions", "invalid data entries", "ok")
          .map { arg =>
            ...
            val tx = InvokeScriptTransaction
              .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, Tac, ts + 4)
              .explicitGet()
            (arg, tx)
          }
    ...
    val scenario =
      for {
        ...
        tTxs = iTxs.takeRight(3).map { tx =>
          TransferTransaction
            .selfSigned(1.toByte, master, invoker.toAddress, IssuedAsset(tx.assetId), ENOUGH_AMT / 2, Tac, fee, ByteStr.empty, ts + 3)
            .explicitGet()
        }
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V3, invoker, master.toAddress, Some(fc), payments, fee, Tac, ts + 6)
          .explicitGet()
  ...
  property("scriptHash") {
    ...
    val scenario =
      for {
        ...
        fc       = Terms.FUNCTION_CALL(FunctionHeader.User("foo"), List.empty)
        payments = List(Payment(10, Tac))
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), payments, fee, Tac, ts + 6)
          .explicitGet()
      ...
      case (genesisTxs, invokeTx, dApp, script) =>
        assertDiffAndState(Seq(TestBlock.create(genesisTxs)), TestBlock.create(Seq(invokeTx), Block.ProtoBlockVersion), fsWithV5) {
          case (diff, bc) =>
            ...
            val hash = ByteStr(com.tacplatform.lang.Global.blake2b256(script.bytes().arr))
      ...
      val expr = {
        val script =
          s"""
             ...
             | @Callable(i)
             | func foo() = {
             |  let b1 = tacBalance(this)
             |  let ob1 = tacBalance(Address(base58'$otherAcc'))
             |  if b1 == b1 && ob1 == ob1
             |  then
             ...
             |     let b2 = tacBalance(this)
             |     let ob2 = tacBalance(Address(base58'$otherAcc'))
      ...
      val expr = {
        val script =
          s"""
             ...
             |       if r1 == r1
             |       then
             |        let b3 = tacBalance(this)
             |        let ob3 = tacBalance(Address(base58'$otherAcc'))
  ...
  property("Check balances in payment and asset scripts") {
    ...
    val transferScript = {
      val script = s"""
                      ...
                      | let startTacBalance = this.issuer.getIntegerValue("startTacBalance")
                      | let startInvokerBalance = this.issuer.getIntegerValue("startInvokerBalance")
                      | let resultInvokerBalance = tacBalance(Address(base58'${invoker.toAddress.stringRepr}')).regular
                      | let issuerBalance = tacBalance(this.issuer)
                      |
                      | assetBalance(this.issuer, this.id) == $ENOUGH_AMT                                     &&
                      | assetBalance(this.issuer, paymentAsset) == $ENOUGH_AMT - $paymentFromClientDAppAmount &&
                      | issuerBalance.regular == startTacBalance                                            &&
      ...
      val expr = {
        val script =
          s"""
             ...
             |
             | @Callable(i)
             | func bar(startInvokerBalance: Int, startTacBalance: Int, startPaymentAssetBalance: Int, paymentAsset: ByteVector) = {
             |   let resultInvokerBalance = tacBalance(Address(base58'${invoker.toAddress.stringRepr}')).regular
             |   let paymentAssetBalance = assetBalance(i.caller, paymentAsset)
             |
             |   if (
             |     startInvokerBalance == resultInvokerBalance         &&
             |     startTacBalance == tacBalance(i.caller).regular &&
      ...
      val expr = {
        val script =
          s"""
             ...
             | @Callable(i)
             | func foo() = {
             |  strict startInvokerBalance = tacBalance(Address(base58'${invoker.toAddress.stringRepr}')).regular
             |  strict startTacBalance = tacBalance(this).regular
             |  strict startPaymentAssetBalance = assetBalance(this, base58'$paymentAsset')
             |
             |  strict r = invoke(
             |    Address(base58'$serviceDAppAddress'),
             |    "bar",
             |    [startInvokerBalance, startTacBalance, startPaymentAssetBalance, base58'$paymentAsset'],
             |    [AttachedPayment(base58'$paymentAsset', $paymentFromClientDAppAmount)]
             |  )
             |
             |  strict resultTacBalance = tacBalance(this).regular
             |  strict resultPaymentAssetBalance = assetBalance(this, base58'$paymentAsset')
             |
             |  if (
             |    startTacBalance == resultTacBalance &&
             ...
             |     IntegerEntry("startTacBalance", startTacBalance),
    ...
    val scenario =
      for {
        ...
        fc                = Terms.FUNCTION_CALL(FunctionHeader.User("foo"), List.empty)
        payments          = List(Payment(paymentFromInvokerAmount, Tac))
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V3, invoker, clientDAppAcc.toAddress, Some(fc), payments, fee, Tac, ts + 6)
          .explicitGet()
      } yield (
