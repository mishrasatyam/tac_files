...
import com.tacplatform.settings.{Constants, GenesisSettings, GenesisTransactionSettings}
...
class TransactionsByAddressSpec extends FreeSpec with ScalaCheckDrivenPropertyChecks with BlockGen with WithDomain with Matchers with NoShrink {
  ...
  val gen = for {
    ...
    genesisBlock = Block
      .genesis(
        GenesisSettings(
          ...
          Constants.TotalTac,
          None,
          Seq(GenesisTransactionSettings(sender.toAddress.stringRepr, Constants.TotalTac)),
          1000,
          1.minute
        )
      )
    ...
    transactions1 <- Gen.listOfN(txCount1, transferGen(sender, Gen.oneOf(recipient1, recipient2).map(_.toAddress), Constants.TotalTac / 2 / txCount1))
    block1 = mkBlock(sender, genesisBlock.id(), transactions1)
    txCount2      <- Gen.choose(10, 50)
    transactions2 <- Gen.listOfN(txCount2, transferGen(sender, Gen.oneOf(recipient1, recipient2).map(_.toAddress), Constants.TotalTac / 2 / txCount2))
