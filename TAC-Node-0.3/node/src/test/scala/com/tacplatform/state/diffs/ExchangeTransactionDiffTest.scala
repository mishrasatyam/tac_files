...
import com.tacplatform.account.{Address, KeyPair, PrivateKey, PublicKey}
...
import com.tacplatform.state.diffs.ExchangeTransactionDiff.getOrderFeePortfolio
...
import com.tacplatform.transaction.assets.exchange.{Order, _}
...
import com.tacplatform.{NoShrink, TestValues, TransactionGen, crypto}
...
class ExchangeTransactionDiffTest
  ...
  private def tacPortfolio(amt: Long) = Portfolio.tac(amt)
  ...
  property("Preserves tac invariant, stores match info, rewards matcher") {
  ...
  property("Preserves assets invariant (matcher's fee in one of the assets of the pair or in Tac), stores match info, rewards matcher") {
  ...
  property("buy tac without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransaction, ExchangeTransaction)] = for {
      buyer  <- accountGen
      seller <- accountGen
      ts     <- timestampGen
      gen1: GenesisTransaction = GenesisTransaction.create(buyer.toAddress, 1 * Constants.UnitsInTac, ts).explicitGet()
      gen2: GenesisTransaction = GenesisTransaction.create(seller.toAddress, ENOUGH_AMT, ts).explicitGet()
      issue1: IssueTransaction <- issueGen(buyer)
      exchange <- Gen.oneOf(
        exchangeV1GeneratorP(buyer, seller, Tac, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000)),
        exchangeV2GeneratorP(buyer, seller, Tac, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000))
      )
    } yield {
  ...
  property("small fee cases") {
    ...
    forAll(preconditions, priceGen) {
      case ((buyer, seller, matcher, gen1, gen2, issue1), price) =>
        val assetPair = AssetPair(IssuedAsset(issue1.id()), Tac)
        val buy = Order.buy(
          Order.V1,
          sender = buyer,
  ...
  property("Diff for ExchangeTransaction works as expected and doesn't use rounding inside") {
    ...
    val assetPair                                          = AssetPair(Tac, IssuedAsset(issue1.id()))
  ...
  property("Disable use Order on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createTacTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createTacTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
  ...
  property("ExchangeTransaction with Orders V4 uses asset decimals for price calculation") {
    ...
    val tacUsdn   = AssetPair(Tac, usdn)
    val tidexTac  = AssetPair(tidex, Tac)
    val liquidTac = AssetPair(liquid, Tac)
    ...
    val scenarios = Table(
      ...
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 55768188998L, 592600L, 592600L, 592600L, tacUsdn),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 55768188998L, 59260000L, 59260000L, 59260000L, tacUsdn),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 55768188998L, 59260000L, 592600L, 59260000L, tacUsdn),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 55768188998L, 59260000L, 59260000L, 592600L, tacUsdn)
      ),
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 213L, 35016774000000L, 35016774000000L, 35016774000000L, tidexTac),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 213L, 35016774L, 35016774L, 35016774L, tidexTac),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 213L, 35016774L, 35016774000000L, 35016774L, tidexTac),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 213L, 35016774L, 35016774L, 35016774000000L, tidexTac)
      ),
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 2000000000L, 13898832L, 13898832L, 13898832L, liquidTac),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 2000000000L, 13898832L, 13898832L, 13898832L, liquidTac),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 2000000000L, 13898832L, 13898832L, 13898832L, liquidTac),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 2000000000L, 13898832L, 13898832L, 13898832L, liquidTac)
      )
    )
    ...
    val scenario =
      for {
        ...
        ttx4 = TransferTransaction
          ...
          .explicitGet()
        assets = Seq(IssuedAsset(itx1.assetId), IssuedAsset(itx2.assetId), Tac)
  ...
  property(s"Accepts failed transactions after ${BlockchainFeatures.BlockV5} activation") {
    val scenario =
      for {
        ...
        eTx <- exchangeGeneratorP(buyer, seller, asset, Tac, fixedMatcher = Some(MATCHER))
          .flatMap { tx =>
            for {
        ...
        buyerBalance   = Map(Tac -> ENOUGH_AMT, asset         -> 0L)
        sellerBalance  = Map(Tac -> (ENOUGH_AMT - fee), asset -> iTx.quantity)
        matcherBalance = Map(Tac -> ENOUGH_AMT, asset         -> 0L)
      ...
      case (exchange, (buyerBalance, sellerBalance, matcherBalance), genesisTxs, throwingScript) =>
        ...
        assertDiffAndState(Seq(TestBlock.create(genesisTxs)), TestBlock.create(Seq(exchange), Block.ProtoBlockVersion), fsWithBlockV5) {
          case (diff, state) =>
            ...
            state.balance(exchange.sender.toAddress, Tac) shouldBe matcherBalance(Tac) - exchange.fee
  ...
  property("Counts exchange fee asset complexity") {
    def test(tradeableAssetIssue: IssueTransaction, feeAssetIssue: IssueTransaction, complexity: Long): Unit = {
      ...
      withDomain(
        ...
        val feeUnits = FeeValidation.getMinFee(d.blockchainUpdater, exchange).explicitGet().minFeeInTac / FeeValidation.FeeUnit
