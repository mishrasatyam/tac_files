...
import com.tacplatform.lang.v1.compiler.Terms.{CONST_BYTESTR, CONST_STRING, FUNCTION_CALL}
      ...
      withDomain(settings) { d =>
        ...
        d.appendBlock(
          Seq(invoker.toAddress, dApp0KP.toAddress, dApp1KP.toAddress)
            .map(addr => GenesisTransaction.create(addr, 10000.tac, ntpTime.getTimestamp()).explicitGet()): _*
        )
        val issueTx = IssueTransaction
          .selfSigned(TxVersion.V3, dApp1KP, "DAPP1", "", 1000_00, 2, false, Some(smartAssetScript), 1.tac, ntpTime.getTimestamp())
          .explicitGet()

        d.appendBlock(
          SetScriptTransaction.selfSigned(TxVersion.V2, dApp0KP, Some(dApp0), 0.01.tac, ntpTime.getTimestamp()).explicitGet(),
          SetScriptTransaction.selfSigned(TxVersion.V2, dApp1KP, Some(dApp1), 0.01.tac, ntpTime.getTimestamp()).explicitGet(),
          issueTx
        )
