...
import com.tacplatform.api.http.requests.{InvokeScriptRequest, SignedInvokeScriptRequest}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.state.diffs.ProduceError.produce
import com.tacplatform.common.utils.{Base64, _}
import com.tacplatform.lang.v1.compiler.Terms
import com.tacplatform.lang.v1.compiler.Terms.{ARR, CONST_BIGINT, CONST_LONG, CaseObj}
import com.tacplatform.lang.v1.compiler.Types.CASETYPEREF
import com.tacplatform.lang.v1.{ContractLimits, FunctionHeader, Serde}
import com.tacplatform.protobuf.transaction._
import com.tacplatform.protobuf.{Amount, transaction}
import com.tacplatform.serialization.Deser
import com.tacplatform.transaction.Asset.{IssuedAsset, Tac}
import com.tacplatform.transaction.TxValidationError.NonPositiveAmount
import com.tacplatform.transaction.smart.InvokeScriptTransaction.Payment
import com.tacplatform.transaction.smart.{InvokeScriptTransaction, Verifier}
import com.tacplatform.{EitherMatchers, TransactionGen, crypto}
...
class InvokeScriptTransactionSpecification extends PropSpec with PropertyChecks with Matchers with EitherMatchers with TransactionGen {
  ...
  property("Signed InvokeScriptTransactionRequest parser") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(1, Tac))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
  ...
  property("can't have zero amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(0, Tac))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
        Proofs(List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T").map(s => ByteStr.decodeBase58(s).get))
    )
    req.toTx shouldBe Left(NonPositiveAmount(0, "Tac"))
    AddressScheme.current = DefaultAddressScheme
  }
  ...
  property("can't have negative amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(-1L, Tac))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
        Proofs(List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T").map(s => ByteStr.decodeBase58(s).get))
    )
    req.toTx shouldBe Left(NonPositiveAmount(-1, "Tac"))
    AddressScheme.current = DefaultAddressScheme
  }
}
