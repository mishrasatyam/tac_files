  ...
  "Invoke state changes" should "include intermediate invokes" in withDomain(DomainPresets.RideV5) { d =>
    ...
    { // Prerequisites
      ...
      val genesis = Seq(
        ...
        TxHelpers.genesis(dAppAddress.toAddress, 1.tac),
        TxHelpers.genesis(addr2f.toAddress, 1.tac),
        TxHelpers.genesis(addr3f.toAddress, 1.tac),
        TxHelpers.genesis(addr2s.toAddress, 1.tac),
        TxHelpers.genesis(addr3s.toAddress, 1.tac)
      )
      val setScripts = Seq(
