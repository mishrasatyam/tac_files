...
import com.tacplatform.transaction.assets.exchange.{AssetPair, Order, OrderType}
import com.tacplatform.transaction.smart.Verifier
import com.tacplatform.{NTPTime, TransactionGen}
  ...
  property("AssetPair test") {
    forAll(assetIdGen, assetIdGen) { (assetA, assetB) =>
      whenever(assetA != assetB) {
        val assetAId: Asset = assetA.fold[Asset](Tac)(arr => IssuedAsset(arr))
        val assetBId: Asset = assetB.fold[Asset](Tac)(arr => IssuedAsset(arr))
