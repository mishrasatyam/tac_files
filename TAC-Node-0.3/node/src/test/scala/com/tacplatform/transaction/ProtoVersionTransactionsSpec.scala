...
class ProtoVersionTransactionsSpec extends FreeSpec with TransactionGen with Matchers {
  ...
  val MinFee: Long            = (0.001 * Constants.UnitsInTac).toLong
  val DataTxFee: Long         = 15000000
  val InvokeScriptTxFee: Long = 15000000
  val MassTransferTxFee: Long = 15000000
  val SetScriptFee: Long      = (0.01 * Constants.UnitsInTac).toLong
  ...
  "all txs" - {
    ...
    "TransferTransaction" in {
      ...
      val transferTx =
        TransferTransaction
          .selfSigned(TxVersion.V3, Account, recipient, asset, 100, Asset.Tac, MinFee, ByteStr(attachment), Now)
          .explicitGet()
    ...
    "MassTransferTransaction" in {
      ...
      val massTransferTx =
        MassTransferTransaction
          .selfSigned(TxVersion.V2, Account, Asset.Tac, transfers, MassTransferTxFee, Now, ByteStr(attachment))
          .explicitGet()
