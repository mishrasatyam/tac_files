...
import com.tacplatform.{EitherMatchers, history}
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with EitherMatchers with HistoryTest with ShouldVerb with WithDomain {
  ...
  private val TacSettings = history.DefaultTacSettings.copy(
    blockchainSettings = history.DefaultTacSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultTacSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultTacSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val TacSettingsWithDoubling = TacSettings.copy(
    blockchainSettings = TacSettings.blockchainSettings.copy(
      functionalitySettings = TacSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(TacSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(TacSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(TacSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(TacSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
