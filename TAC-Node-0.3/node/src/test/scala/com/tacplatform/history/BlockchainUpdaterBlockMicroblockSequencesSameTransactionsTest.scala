...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, TransferTransaction, Int)] = for {
      ...
      payment: TransferTransaction = createTacTransfer(master, master.toAddress, amt, fee, ts).explicitGet()
    ...
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, Seq[Seq[TransferTransaction]], Int)] =
      for {
        ...
        microBlockTxs = (1 to txCount * microBlockCount)
          .map(step => createTacTransfer(master, master.toAddress, amt, fee, ts + step).explicitGet())
  ...
  def randomPayment(accs: Seq[KeyPair], ts: Long): Gen[TransferTransaction] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createTacTransfer(from, to.toAddress, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[KeyPair], KeyPair, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice.toAddress, TOTAL_TAC / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob.toAddress, TOTAL_TAC / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie.toAddress, TOTAL_TAC / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave.toAddress, TOTAL_TAC / 4, ts + 4).explicitGet()
    } yield (
