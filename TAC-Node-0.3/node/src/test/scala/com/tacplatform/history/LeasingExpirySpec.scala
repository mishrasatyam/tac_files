...
import com.tacplatform.account.{AddressOrAlias, Alias, KeyPair}
...
import com.tacplatform.state.{Blockchain, LeaseBalance}
...
class LeasingExpirySpec
  ...
  private val genesis = for {
    ...
    maxFeeAmount <- Gen.choose(100000L, 1 * Constants.UnitsInTac)
    ...
    genesisBlock = TestBlock.create(
      ts,
      Seq(
        GenesisTransaction.create(lessor.toAddress, Constants.TotalTac * Constants.UnitsInTac, ntpTime.getTimestamp()).explicitGet(),
        transfer,
        createAlias
      )
    )
  ...
  private def lease(sender: KeyPair, recipient: AddressOrAlias): Gen[LeaseTransaction] =
    for {
      amount <- Gen.choose(1 * Constants.UnitsInTac, 1000 * Constants.UnitsInTac)
      fee    <- smallFeeGen
  ...
  "Leasing cancellation" - {
    val amount     = 1000 * Constants.UnitsInTac
