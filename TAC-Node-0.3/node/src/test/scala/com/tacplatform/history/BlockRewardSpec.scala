...
import com.tacplatform.api.http.RewardApiRoute
import com.tacplatform.block.Block
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.EitherExt2
import com.tacplatform.database.Keys
...
import com.tacplatform.settings.{Constants, FunctionalitySettings, RewardsSettings}
...
import com.tacplatform.{EitherMatchers, NoShrink, TransactionGen}
...
class BlockRewardSpec
  ...
  private val InitialReward               = 6 * Constants.UnitsInTac
  ...
  private def mkEmptyBlockIncReward(ref: ByteStr, signer: KeyPair): Block =
    TestBlock.create(ntpNow, ref, Seq.empty, signer, rewardVote = InitialReward + 1 * Constants.UnitsInTac, version = Block.RewardBlockVersion)
  ...
  private val InitialMinerBalance = 10000 * Constants.UnitsInTac
  ...
  private val genesis = for {
    ...
    genesisBlock = TestBlock.create(
      ...
      Seq(
        GenesisTransaction
          .create(sourceAddress.toAddress, (Constants.TotalTac - 60000) * Constants.UnitsInTac, ntpTime.getTimestamp())
          .explicitGet(),
        GenesisTransaction.create(issuer.toAddress, 40000 * Constants.UnitsInTac, ntpTime.getTimestamp()).explicitGet(),
  ...
  private val activationScenario = for {
    ...
    transfers                                  <- Gen.listOfN(10, transferGeneratorP(ntpNow, sourceAddress, recipient.toAddress, 1000 * Constants.UnitsInTac))
  ...
  "Miner receives reward as soon as the feature is activated and changes reward amount after voting" in forAll(activationScenario) {
    case (miner, transfers, b1s, b2s, activationBlock, b3s, b4s, newTermBlock, b5s, b6s, b7s) =>
      withDomain(rewardSettings) { d =>
        ...
        val NextReward = InitialReward + 1 * Constants.UnitsInTac
        ...
        val DecreasedReward = InitialReward - 1 * Constants.UnitsInTac
  ...
  "Miner receives reward and fees" - {
    ...
    "when NG state is empty" in forAll(ngEmptyScenario) {
      case (miner1, miner2, b2s, b3, m3s) =>
        withDomain(rewardSettings) { d =>
          ...
          d.db.get(Keys.blockMetaAt(Height(BlockRewardActivationHeight - 1))).map(_.totalFeeInTac) shouldBe OneTotalFee.some
          ...
          d.blockchainUpdater.liquidBlockMeta.map(_.totalFeeInTac) shouldBe 0L.some
          ...
          d.blockchainUpdater.liquidBlockMeta.map(_.totalFeeInTac) shouldBe OneTotalFee.some
    ...
    "when all blocks without fees" in forAll(blockWithoutFeesScenario) {
      case (miner1, miner2, b1s, b2, b3s, b4) =>
        withDomain(rewardSettings) { d =>
          val initialTacAmount = BigInt(Constants.TotalTac) * BigInt(Constants.UnitsInTac)
          ...
          d.blockchainUpdater.tacAmount(BlockRewardActivationHeight - 1) shouldBe initialTacAmount
          ...
          d.blockchainUpdater.height shouldBe BlockRewardActivationHeight
          d.blockchainUpdater.tacAmount(BlockRewardActivationHeight) shouldBe initialTacAmount + InitialReward
          ...
          b3s.zipWithIndex.foreach {
            case (b, i) =>
              ...
              d.blockchainUpdater.tacAmount(BlockRewardActivationHeight + i + 1) shouldBe initialTacAmount + BigInt(InitialReward * (i + 2))
          ...
          d.blockchainUpdater.tacAmount(BlockRewardActivationHeight + term) shouldBe initialTacAmount + term * InitialReward + InitialReward + minIncrement
  ...
  private val calcRewardSettings = rewardSettings.copy(
    blockchainSettings = rewardSettings.blockchainSettings.copy(
      ...
      rewardsSettings = RewardsSettings(12, 6 * Constants.UnitsInTac, 1 * Constants.UnitsInTac, 6)
    )
  )
  ...
  "Reward calculated correctly" in forAll(calcScenario) {
    case (b1s, b2, b3s, b4) =>
      withDomain(calcRewardSettings) { d =>
        ...
        d.blockchainUpdater.blockReward(9) shouldBe (6 * Constants.UnitsInTac).some
        d.blockchainUpdater.blockReward(15) shouldBe (6 * Constants.UnitsInTac).some

        d.blockchainUpdater.processBlock(b4) should beRight
        d.blockchainUpdater.blockReward(16) shouldBe (7 * Constants.UnitsInTac).some
  ...
  private val smallPeriodRewardSettings = rewardSettings.copy(
    blockchainSettings = rewardSettings.blockchainSettings.copy(
      ...
      rewardsSettings = RewardsSettings(3, 6 * Constants.UnitsInTac, 1 * Constants.UnitsInTac, 2)
    )
  )
  ...
  "Reward calculated correctly for small voting period" in forAll(smallCalcScenario) {
    case (b1s, b2, b3s) =>
      withDomain(smallPeriodRewardSettings) { d =>
        ...
        d.blockchainUpdater.blockReward(7) shouldBe (7 * Constants.UnitsInTac).some
      }
  }
}
