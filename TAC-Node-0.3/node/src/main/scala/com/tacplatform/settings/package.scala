...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(external.getConfig("tac.defaults"))
    ...
    val networkDefaults = {
      val network = withApp.getString("tac.blockchain.type").toLowerCase
      withApp.getConfig(s"tac.defaults.$network")
    }

    external
      .withFallback(cmdDefaults)
      .withFallback(networkDefaults.atKey("tac"))
      .withFallback(ConfigFactory.parseString(s"tac.directory = ${defaultDirectory(withApp)}"))
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("tac.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("tac.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)
