...
import com.tacplatform.account.{KeyPair, PrivateKey, PublicKey}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, Base64, FastBase58}
import com.tacplatform.features.EstimatorProvider._
import com.tacplatform.lang.script.{Script, ScriptReader}
import com.tacplatform.settings.TacSettings
import com.tacplatform.transaction.TransactionFactory
import com.tacplatform.transaction.smart.script.ScriptCompiler
import com.tacplatform.wallet.Wallet
import com.tacplatform.{Application, Version}
...
object UtilApp {
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("tac util"),
      head("Tac Util", Version.VersionString),
      OParser.sequence(
  ...
  private[this] object Actions {
    ...
    def doCompile(settings: TacSettings)(c: Command, str: Array[Byte]): ActionResult = {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.tacplatform.crypto.sign(c.signOptions.privateKey, data).arr)

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.tacplatform.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.tacplatform.crypto.fastHash(data))
      case "secure" => Right(com.tacplatform.crypto.secureHash(data))
