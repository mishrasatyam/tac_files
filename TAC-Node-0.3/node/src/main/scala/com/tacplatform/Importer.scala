...
import com.tacplatform.Exporter.Formats
import com.tacplatform.account.Address
import com.tacplatform.api.common.{CommonAccountsApi, CommonAssetsApi, CommonBlocksApi, CommonTransactionsApi}
import com.tacplatform.block.{Block, BlockHeader}
import com.tacplatform.common.state.ByteStr
import com.tacplatform.consensus.PoSSelector
import com.tacplatform.database.{DBExt, KeyTags, openDB}
...
object Importer extends ScorexLogging {
  ...
  def parseOptions(args: Array[String]): ImportOptions = {
    lazy val commandParser = {
      ...
      OParser.sequence(
        programName("tac import"),
        head("Tac Blockchain Importer", Version.VersionString),
        opt[File]('c', "config")
  ...
  def loadSettings(file: Option[File]): TacSettings = {
  ...
  def initExtensions(
    ...
    if (tacSettings.extensions.isEmpty) Seq.empty
    else {
      val extensionContext: Context = {
        new Context {
          override def settings: TacSettings = tacSettings
      ...
      val extensions = tacSettings.extensions.map { extensionClassName =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    def initFileStream(file: String, offset: Long): InputStream = {
      ...
      file match {
        ...
        case _ =>
          System.setProperty("http.agent", s"tac-node/${Version.VersionString}")
          try {
    ...
    val actorSystem = ActorSystem("tacplatform-import")
      ...
      case Formats.Binary =>
        ...
        db.iterateOver(KeyTags.BlockInfoAtHeight) { e =>
          e.getKey match {
            ...
            case _ =>
              val meta = com.tacplatform.database.readBlockMeta(e.getValue)
              result += meta.size + 4
          }
        }
        result
