...
import com.tacplatform.database.patch.DisableHijackedAliases
import com.tacplatform.database.protobuf.TransactionMeta
import com.tacplatform.features.BlockchainFeatures
import com.tacplatform.lang.ValidationError
import com.tacplatform.protobuf.transaction.PBTransactions
import com.tacplatform.settings.{BlockchainSettings, DBSettings, TacSettings}
import com.tacplatform.state.{TxNum, _}
...
import com.tacplatform.transaction.TxValidationError.{AliasDoesNotExist, AliasIsDisabled}
...
import com.tacplatform.utils.{LoggerFacade, ScorexLogging}
...
object LevelDBWriter extends ScorexLogging {
  ...
  def apply(db: DB, spendableBalanceChanged: Observer[(Address, Asset)], settings: TacSettings): LevelDBWriter with AutoCloseable = {
    ...
    val _tacBalanceFilter = load("tac-balances", KeyTags.TacBalanceHistory)
    ...
    new LevelDBWriter(db, spendableBalanceChanged, settings.blockchainSettings, settings.dbSettings) with AutoCloseable {
      ...
      override val tacBalanceFilter: BloomFilter = _tacBalanceFilter.getOrElse(BloomFilter.AlwaysEmpty)
      ...
      override def close(): Unit = {
        ...
        _tacBalanceFilter.foreach(_.save(lastHeight))
        _assetBalanceFilter.foreach(_.save(lastHeight))
      }
    }
  }

  def readOnly(db: DB, settings: TacSettings): LevelDBWriter = {
    val expectedHeight = loadHeight(db)
    ...
    new LevelDBWriter(db, Observer.stopped, settings.blockchainSettings, settings.dbSettings) {
      ...
      override val tacBalanceFilter: BloomFilter = loadFilter("tac-balances")
...
abstract class LevelDBWriter private[database](
  ...
  def tacBalanceFilter: BloomFilter
  ...
  protected override def loadBalance(req: (Address, Asset)): Long =
    addressId(req._1).fold(0L) { addressId =>
      req._2 match {
        ...
        case Tac =>
          val kwbh = Keys.tacBalanceHistory(addressId)
          if (tacBalanceFilter.mightContain(kwbh.suffix))
            writableDB.readOnly(_.fromHistory(kwbh, Keys.tacBalance(addressId))).getOrElse(0L)
          else 0L
      }
    }
  ...
  private[database] def loadLposPortfolio(db: ReadOnlyDB, addressId: AddressId) = Portfolio(
    db.fromHistory(Keys.tacBalanceHistory(addressId), Keys.tacBalance(addressId)).getOrElse(0L),
  ...
  override def tacAmount(height: Int): BigInt = readOnly { db =>
    val factHeight = height.min(this.height)
    if (db.has(Keys.tacAmount(factHeight))) db.get(Keys.tacAmount(factHeight))
  ...
  private def appendBalances(
    ...
    for ((addressId, updatedBalances) <- balances) {
      for ((asset, balance) <- updatedBalances) {
        asset match {
          case Tac =>
            rw.put(Keys.tacBalance(addressId)(height), balance)
            val kwbh = Keys.tacBalanceHistory(addressId)
            if (tacBalanceFilter.mightContain(kwbh.suffix))
              updateHistory(rw, kwbh, balanceThreshold, Keys.tacBalance(addressId)).foreach(rw.delete)
            else {
              rw.put(kwbh, Seq(height))
              tacBalanceFilter.put(kwbh.suffix)
            }
          case a: IssuedAsset =>
  ...
  override protected def doAppend(
    ...
    readWrite { rw =>
      ...
      reward.foreach { lastReward =>
        ...
        rw.put(Keys.tacAmount(height), tacAmount(height - 1) + lastReward)
      }
    ...
    val discardedBlocks: Seq[(Block, ByteStr)] = for (currentHeight <- height until targetHeight by -1) yield {
      ...
      val discardedBlock = readWrite { rw =>
        ...
        for ((addressId, address) <- changedAddresses) {
          ...
          balancesToInvalidate += (address -> Tac)
          rw.delete(Keys.tacBalance(addressId)(currentHeight))
          rw.filterHistory(Keys.tacBalanceHistory(addressId), currentHeight)
        ...
        rw.delete(Keys.tacAmount(currentHeight))
  ...
  override def balanceAtHeight(address: Address, height: Int, assetId: Asset = Tac): Option[(Int, Long)] = readOnly { db =>
    db.get(Keys.addressId(address)).flatMap { addressId =>
      assetId match {
        case Tac =>
          closest(db.get(Keys.tacBalanceHistory(addressId)), height).map { wh =>
            val b: Long = db.get(Keys.tacBalance(addressId)(wh))
            (wh, b)
          }
        case asset @ IssuedAsset(_) =>
  ...
  override def balanceSnapshots(address: Address, from: Int, to: Option[BlockId]): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      ...
      val wbh      = slice(db.get(Keys.tacBalanceHistory(addressId)), from, toHeigth)
      val lbh      = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, toHeigth)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.tacBalance(addressId)(wh)))
  ...
  private def transactionsAtHeight(h: Height): List[(TxNum, Transaction)] = readOnly { db =>
    import com.tacplatform.protobuf.transaction.PBSignedTransaction
