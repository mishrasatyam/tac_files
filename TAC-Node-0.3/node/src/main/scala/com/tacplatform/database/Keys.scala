...
object Keys {
  ...
  def tacBalanceHistory(addressId: AddressId): Key[Seq[Int]] = historyKey(TacBalanceHistory, addressId.toByteArray)

  def tacBalance(addressId: AddressId)(height: Int): Key[Long] =
    Key(TacBalance, hAddr(height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  def tacAmount(height: Int): Key[BigInt] = Key(TacAmount, h(height), Option(_).fold(BigInt(0))(BigInt(_)), _.toByteArray)
