...
import com.tacplatform.protobuf.transaction.Transaction.Data
import com.tacplatform.serialization.Deser
import com.tacplatform.state.{BinaryDataEntry, BooleanDataEntry, EmptyDataEntry, IntegerDataEntry, StringDataEntry}
...
import com.tacplatform.transaction.{Proofs, TxValidationError}
import com.tacplatform.utils.StringBytes
import com.tacplatform.{transaction => vt}

import scala.util.Try

object PBTransactions {
  import com.tacplatform.protobuf.utils.PBImplicitConversions._
  ...
  def create(
      sender: com.tacplatform.account.PublicKey,
      chainId: Byte = 0,
      fee: Long = 0L,
      feeAssetId: VanillaAssetId = Tac,
      timestamp: Long = 0L,
      version: Int = 0,
      proofsArray: Seq[com.tacplatform.common.state.ByteStr] = Nil,
      data: com.tacplatform.protobuf.transaction.Transaction.Data = com.tacplatform.protobuf.transaction.Transaction.Data.Empty
  ): SignedTransaction =
    new SignedTransaction(
  ...
  def protobuf(tx: VanillaTransaction): PBSignedTransaction = {
    tx match {
      ...
      case vt.PaymentTransaction(sender, recipient, amount, fee, timestamp, signature, chainId) =>
        ...
        PBTransactions.create(sender, chainId, fee, Tac, timestamp, 1, Seq(signature), Data.Payment(data))
  ...
  def toVanillaDataEntry(de: DataTransactionData.DataEntry): com.tacplatform.state.DataEntry[_] = {
  ...
  def toPBDataEntry(de: com.tacplatform.state.DataEntry[_]): DataTransactionData.DataEntry = {
  ...
  def toVanillaScript(script: ByteString): Option[com.tacplatform.lang.script.Script] = {
    import com.tacplatform.common.utils._
    if (script.isEmpty) None else Some(ScriptReader.fromBytes(script.toByteArray).explicitGet())
  }

  def toPBScript(script: Option[com.tacplatform.lang.script.Script]): ByteString = script match {
