...
import com.tacplatform.features.BlockchainFeatures.{BlockV5, SynchronousCalls}
import com.tacplatform.features.EstimatorProvider._
import com.tacplatform.features.InvokeScriptSelfPaymentPolicyProvider._
import com.tacplatform.features.ScriptTransferValidationProvider._
import com.tacplatform.lang._
...
import com.tacplatform.lang.v1.compiler.Terms.{FUNCTION_CALL, _}
import com.tacplatform.lang.v1.traits.Environment
import com.tacplatform.lang.v1.traits.domain.{AssetTransfer, _}
import com.tacplatform.lang.v1.traits.domain.Tx.{BurnPseudoTx, ReissuePseudoTx, ScriptTransfer, SponsorFeePseudoTx}
import com.tacplatform.settings.Constants
import com.tacplatform.state._
import com.tacplatform.state.diffs.DiffsCommon
import com.tacplatform.state.diffs.FeeValidation._
import com.tacplatform.state.reader.CompositeBlockchain
import com.tacplatform.transaction.{Asset, AssetIdLength}
...
import com.tacplatform.transaction.smart.script.trace.{AssetVerifierTrace, TracedResult}
import com.tacplatform.transaction.smart.script.trace.AssetVerifierTrace.AssetContext
import com.tacplatform.transaction.validation.impl.{LeaseCancelTxValidator, LeaseTxValidator, SponsorFeeTxValidator}
...
object InvokeDiffsCommon {
  def txFeeDiff(blockchain: Blockchain, tx: InvokeScriptTransaction): Either[GenericError, (Long, Map[Address, Portfolio])] = {
    ...
    tx.assetFee._1 match {
      case Tac => Right((attachedFee, Map(tx.sender.toAddress -> Portfolio(-attachedFee))))
      case asset @ IssuedAsset(_) =>
        for {
          assetInfo <- blockchain
            .assetDescription(asset)
            .toRight(GenericError(s"Asset $asset does not exist, cannot be used to pay fees"))
          feeInTac <- Either.cond(
            assetInfo.sponsorship > 0,
            Sponsorship.toTac(attachedFee, assetInfo.sponsorship),
            GenericError(s"Asset $asset is not sponsored, cannot be used to pay fees")
          )
        } yield {
          val portfolioDiff =
            Map(tx.sender.toAddress          -> Portfolio(assets = Map(asset              -> -attachedFee))) |+|
              Map(assetInfo.issuer.toAddress -> Portfolio(-feeInTac, assets = Map(asset -> attachedFee)))
          (feeInTac, portfolioDiff)
        }
    }
  }
  ...
  def calcAndCheckFee[E <: ValidationError](
    ...
    TracedResult {
      ...
      for {
        (attachedFeeInTac, portfolioDiff) <- txFeeDiff(blockchain, tx)
        _ <- {
