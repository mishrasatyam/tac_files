package com.tacplatform.state.reader
...
final case class CompositeBlockchain(
  ...
  override def balanceAtHeight(address: Address, h: Int, assetId: Asset = Tac): Option[(Int, Long)] = {
  ...
  override def tacAmount(height: Int): BigInt = inner.tacAmount(height) + BigInt(reward.getOrElse(0L))
