...
import com.tacplatform.metrics.TxProcessingStats
import com.tacplatform.metrics.TxProcessingStats.TxTimerExt
import com.tacplatform.state.{Blockchain, Diff, InvokeScriptResult, LeaseBalance, NewTransactionInfo, Portfolio, Sponsorship}
...
import com.tacplatform.transaction.smart.{InvokeScriptTransaction, SetScriptTransaction, Verifier}
import com.tacplatform.transaction.smart.script.trace.{TracedResult, TraceStep}
  ...
  private def feePortfolios(blockchain: Blockchain, tx: Transaction): Either[ValidationError, Map[Address, Portfolio]] =
    tx match {
      ...
      case ptx: ProvenTransaction =>
        ptx.assetFee match {
          case (Tac, fee) => Map(ptx.sender.toAddress -> Portfolio(-fee, LeaseBalance.empty, Map.empty)).asRight
          case (asset @ IssuedAsset(_), fee) =>
            for {
              ...
              tacFee <- Either.cond(
                ...
                Sponsorship.toTac(fee, assetInfo.sponsorship),
                GenericError(s"Asset $asset is not sponsored, cannot be used to pay fees")
              )
            } yield Monoid.combine(
              Map(ptx.sender.toAddress       -> Portfolio(0, LeaseBalance.empty, Map(asset         -> -fee))),
              Map(assetInfo.issuer.toAddress -> Portfolio(-tacFee, LeaseBalance.empty, Map(asset -> fee)))
            )
        }
