...
import com.tacplatform.account.{Address, AddressOrAlias, Alias, PublicKey}
...
object Sponsorship {
  ...
  def calcTacFeeAmount(tx: Transaction, getSponsorship: IssuedAsset => Option[Long]): Long = tx.assetFee match {
    case (asset @ IssuedAsset(_), amountInAsset) =>
      val sponsorship = getSponsorship(asset).getOrElse(0L)
      Sponsorship.toTac(amountInAsset, sponsorship)

    case (Asset.Tac, amountInTac) =>
      amountInTac
  }
  ...
  def fromTac(tacFee: Long, sponsorship: Long): Long =
    if (tacFee == 0 || sponsorship == 0) 0
    else {
      val assetFee = BigInt(tacFee) * sponsorship / FeeValidation.FeeUnit
