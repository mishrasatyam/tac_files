...
import com.tacplatform.transaction.assets.exchange.{ExchangeTransaction, Order, OrderType}
...
object ExchangeTransactionDiff {
  ...
  private[diffs] def getSpendAmount(
    ...
    Try {
      if (order.orderType == OrderType.SELL) matchAmount
        ...
        if (order.getSpendAssetId == Tac && !(spend + order.matcherFee).isValidLong) {
  ...
  /**
    * Calculates fee portfolio from the order (taking into account that in OrderV3 fee can be paid in asset != Tac)
    */
