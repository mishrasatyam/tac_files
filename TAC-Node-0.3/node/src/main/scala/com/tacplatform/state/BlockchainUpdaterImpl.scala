...
import com.tacplatform.block.{Block, MicroBlock, SignedBlockHeader}
import com.tacplatform.block.Block.BlockId
import com.tacplatform.common.state.ByteStr
import com.tacplatform.database.Storage
import com.tacplatform.events.BlockchainUpdateTriggers
import com.tacplatform.features.BlockchainFeatures
import com.tacplatform.lang.ValidationError
import com.tacplatform.metrics.{TxsInBlockchainStats, _}
import com.tacplatform.mining.{Miner, MiningConstraint, MiningConstraints}
import com.tacplatform.settings.{BlockchainSettings, TacSettings}
import com.tacplatform.state.diffs.BlockDiffer
import com.tacplatform.state.reader.{CompositeBlockchain, LeaseDetails}
import com.tacplatform.transaction._
import com.tacplatform.transaction.Asset.{IssuedAsset, Tac}
import com.tacplatform.transaction.TxValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
import com.tacplatform.transaction.lease._
import com.tacplatform.transaction.transfer.TransferTransaction
import com.tacplatform.utils.{forceStopApplication, ScorexLogging, Time, UnsupportedFeature}
import kamon.Kamon
...
class BlockchainUpdaterImpl(
  ...
  import com.tacplatform.state.BlockchainUpdaterImpl._
  import tacSettings.blockchainSettings.functionalitySettings
  ...
  private lazy val maxBlockReadinessAge = tacSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis
  ...
  override val settings: BlockchainSettings = tacSettings.blockchainSettings
  ...
  private def featuresApprovedWithBlock(block: Block): Set[Short] = {
    ...
    if (height % featuresCheckPeriod == 0) {
      ...
      if (unimplementedActivated.nonEmpty) {
        ...
        if (tacSettings.featuresSettings.autoShutdownOnUnsupportedFeature) {
  ...
  override def processBlock(block: Block, hitSource: ByteStr, verify: Boolean = true): Either[ValidationError, Seq[Diff]] =
    writeLock {
      ...
      Either
        .cond(
          !tacSettings.featuresSettings.autoShutdownOnUnsupportedFeature || notImplementedFeatures.isEmpty,
          (),
              ...
              _ map {
                case (BlockDiffer.Result(newBlockDiff, carry, totalFee, updatedTotalConstraint, _), discDiffs, reward, hitSource) =>
                  ...
                  if ((block.header.timestamp > time
                        .getTimestamp() - tacSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis) || (newHeight % 100 == 0)) {
  ...
  override def tacAmount(height: Int): BigInt = readLock {
    ngState match {
      case Some(ng) if this.height == height =>
        leveldb.tacAmount(height - 1) + BigInt(ng.reward.getOrElse(0L))
      case _ =>
        leveldb.tacAmount(height)
    }
  }
  ...
  override def balanceAtHeight(address: Address, h: Int, assetId: Asset = Tac): Option[(Int, Long)] = readLock {
