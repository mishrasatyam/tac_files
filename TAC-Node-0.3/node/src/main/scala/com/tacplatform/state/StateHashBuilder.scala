...
import com.tacplatform.state.StateHashBuilder.Result
...
class StateHashBuilder {
  ...
  def addTacBalance(address: Address, balance: Long): Unit = {
    addEntry(SectionId.TacBalance, address.bytes)(Longs.toByteArray(balance))
  }
