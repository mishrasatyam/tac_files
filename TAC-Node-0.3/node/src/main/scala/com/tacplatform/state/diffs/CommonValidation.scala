...
import com.tacplatform.features.OverdraftValidationProvider._
    ...
    if (blockTime >= blockchain.settings.functionalitySettings.allowTemporaryNegativeUntil) {
      def checkTransfer(
        ...
        val amountDiff = assetId match {
          ...
          case Tac                => Portfolio(-amount, LeaseBalance.empty, Map.empty)
        }
        val feeDiff = feeAssetId match {
          case aid @ IssuedAsset(_) => Portfolio(0, LeaseBalance.empty, Map(aid -> -feeAmount))
          case Tac                => Portfolio(-feeAmount, LeaseBalance.empty, Map.empty)
        }

        val spendings       = Monoid.combine(amountDiff, feeDiff)
        val oldTacBalance = blockchain.balance(sender, Tac)

        val newTacBalance     = oldTacBalance + spendings.balance
        val feeUncheckedBalance = oldTacBalance + amountDiff.balance

        val overdraftFilter = allowFeeOverdraft && feeUncheckedBalance >= 0
        if (!overdraftFilter && newTacBalance < 0) {
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative tac balance to (at least) temporary negative state, current balance equals $oldTacBalance, " +
                s"spends equals ${spendings.balance}, result is $newTacBalance"
            )
          )
        } else {
      ...
      tx match {
        case ptx: PaymentTransaction if blockchain.balance(ptx.sender.toAddress, Tac) < (ptx.amount + ptx.fee) =>
          Left(
            GenericError(
              "Attempt to pay unavailable funds: balance " +
                s"${blockchain.balance(ptx.sender.toAddress, Tac)} is less than ${ptx.amount + ptx.fee}"
            )
          )
        case ttx: TransferTransaction     => checkTransfer(ttx.sender.toAddress, ttx.assetId, ttx.amount, ttx.feeAssetId, ttx.fee)
        case mtx: MassTransferTransaction => checkTransfer(mtx.sender.toAddress, mtx.assetId, mtx.transfers.map(_.amount).sum, Tac, mtx.fee)
