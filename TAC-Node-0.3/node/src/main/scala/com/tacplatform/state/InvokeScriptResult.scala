...
import com.tacplatform.account.{Address, AddressOrAlias, AddressScheme, Alias}
...
import com.tacplatform.lang.v1.evaluator.{IncompleteResult, ScriptResult, ScriptResultV3, ScriptResultV4}
import com.tacplatform.lang.v1.traits.domain._
import com.tacplatform.protobuf.{Amount, _}
import com.tacplatform.protobuf.transaction.{PBAmounts, PBRecipients, PBTransactions, InvokeScriptResult => PBInvokeScriptResult}
import com.tacplatform.protobuf.utils.PBUtils
import com.tacplatform.state.{InvokeScriptResult => R}
...
object InvokeScriptResult {
  type LeaseCancel = com.tacplatform.lang.v1.traits.domain.LeaseCancel
  type SponsorFee = com.tacplatform.lang.v1.traits.domain.SponsorFee
  type Issue = com.tacplatform.lang.v1.traits.domain.Issue
  type Reissue = com.tacplatform.lang.v1.traits.domain.Reissue
  type Burn = com.tacplatform.lang.v1.traits.domain.Burn
  type DataEntry = com.tacplatform.state.DataEntry[_]
  ...
  def paymentsFromPortfolio(addr: Address, portfolio: Portfolio): Seq[Payment] = {
    val tac  = InvokeScriptResult.Payment(addr, Tac, portfolio.balance)
    val assets = portfolio.assets.map { case (assetId, amount) => InvokeScriptResult.Payment(addr, assetId, amount) }
    (assets.toVector ++ Some(tac)).filter(_.amount != 0)
  }
  ...
  def fromLangResult(invokeId: ByteStr, result: ScriptResult): InvokeScriptResult = {
    import com.tacplatform.lang.v1.traits.{domain => lang}
              ...
              (payments.map {
                case CaseObj(t, fields) =>
                  (fields("assetId"), fields("amount")) match {
                    ...
                    case (_, CONST_LONG(a))                => InvokeScriptResult.AttachedPayment(Tac, a)
                  }
              }),
