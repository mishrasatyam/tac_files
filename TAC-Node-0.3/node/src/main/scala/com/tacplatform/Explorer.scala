...
import com.tacplatform.api.common.AddressPortfolio
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, Base64, EitherExt2}
import com.tacplatform.database.{DBExt, KeyTags, Keys, LevelDBWriter, openDB, readTransactionBytes}
import com.tacplatform.protobuf.transaction.{PBSignedTransaction, PBTransactions}
import com.tacplatform.settings.Constants
import com.tacplatform.state.{Blockchain, Diff, Height, Portfolio, TxNum}
...
object Explorer extends ScorexLogging {
  ...
  def main(argsRaw: Array[String]): Unit = {
    if (argsRaw.isEmpty || Seq("-h", "--help").exists(argsRaw.contains)) {
      System.err.println("Usage: tac explore <command> [args] [--config|-c <cfg file>]")
      return
    }
    ...
    try {
      ...
      flag match {
        case "WB" =>
          ...
          db.iterateOver(KeyTags.TacBalance) { e =>
          ...
          val expectedTotalBalance = Constants.UnitsInTac * Constants.TotalTac + actualTotalReward
          val byKeyTotalBalance    = reader.tacAmount(blockchainHeight)

          if (actualTotalBalance != expectedTotalBalance || expectedTotalBalance != byKeyTotalBalance)
            log.error(
              s"Something wrong, actual total tac balance: $actualTotalBalance," +
                s" expected total tac balance: $expectedTotalBalance, total tac balance by key: $byKeyTotalBalance"
            )
          else
            log.info(s"Correct total tac balance: $actualTotalBalance TACLETS")
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.tacBalance(addressId)(h)
        ...
        case "AP" =>
          ...
          log.info(s"$address : ${pf.balance} TAC, ${pf.lease}, ${pf.assets.size} assets")
