...
import com.tacplatform.api.http.TransactionsApiRoute.TransactionJsonSerializer
import com.tacplatform.common.state.ByteStr
import com.tacplatform.lang.ValidationError
import com.tacplatform.mining.{Miner, MinerDebugInfo}
import com.tacplatform.network.{PeerDatabase, PeerInfo, _}
import com.tacplatform.settings.{RestAPISettings, TacSettings}
import com.tacplatform.state.{Blockchain, LeaseBalance, NG, Portfolio, StateHash}
...
import com.tacplatform.transaction.TxValidationError.{GenericError, InvalidRequestSignature}
import com.tacplatform.transaction.smart.script.trace.TracedResult
import com.tacplatform.utils.{ScorexLogging, Time}
...
case class DebugApiRoute(
  ...
  private lazy val tacConfig: JsObject = Json.obj("tac" -> (fullConfig \ "tac").get)
  ...
  override lazy val route: Route = pathPrefix("debug") {
    stateChanges ~ balanceHistory ~ stateHash ~ validate ~ withAuth {
      state ~ info ~ stateTac ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ configInfo ~ print
    }
  }
  ...
  def stateTac: Route = (path("stateTac" / IntNumber) & get) { height =>
    distribution(height)
  }
  ...
  def configInfo: Route = (path("configInfo") & get & parameter("full".as[Boolean])) { full =>
    complete(if (full) fullConfig else tacConfig)
  }
