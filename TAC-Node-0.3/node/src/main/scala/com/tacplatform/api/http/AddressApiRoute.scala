...
import com.tacplatform.api.http.requests.DataRequest
import com.tacplatform.common.state.ByteStr
import com.tacplatform.common.utils.{Base58, Base64}
...
import com.tacplatform.lang.contract.meta.FunctionSignatures
...
import com.tacplatform.state.{Blockchain, DataEntry}
...
import com.tacplatform.transaction.{Asset, TransactionFactory}
import com.tacplatform.utils.{Time, _}
...
case class AddressApiRoute(
  ...
  def balances: Route = (path("balance") & get & parameters(("height".as[Int].?, "address".as[String].*, "asset".?))) {
    (maybeHeight, addresses, assetId) =>
      ...
      validateBalanceDepth(height)(
        complete(
          balancesJson(height, addresses.toSeq, assetId.fold(Tac: Asset)(a => IssuedAsset(ByteStr.decodeBase58(a).get)))
        )
      )
  }
  ...
  def balancesPost: Route = (path("balance") & (post & entity(as[JsObject]))) { request =>
    ...
    validateBalanceDepth(height)(complete(balancesJson(height, addresses, assetId.fold(Tac: Asset)(a => IssuedAsset(ByteStr.decodeBase58(a).get)))))
  }
