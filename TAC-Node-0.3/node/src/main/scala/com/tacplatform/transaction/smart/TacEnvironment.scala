...
import com.tacplatform.account
import com.tacplatform.account.AddressOrAlias
import com.tacplatform.block.BlockHeader
...
import com.tacplatform.lang.v1.compiler.Terms.{EVALUATED, FUNCTION_CALL}
import com.tacplatform.lang.v1.traits._
import com.tacplatform.lang.v1.traits.domain._
import com.tacplatform.lang.v1.traits.domain.Recipient._
import com.tacplatform.state._
import com.tacplatform.state.diffs.invoke.{InvokeScript, InvokeScriptDiff}
...
import com.tacplatform.transaction.TxValidationError.{FailedTransactionError, GenericError}
import com.tacplatform.transaction.assets.exchange.Order
import com.tacplatform.transaction.serialization.impl.PBTransactionSerializer
import com.tacplatform.transaction.smart.InvokeScriptTransaction.Payment
import com.tacplatform.transaction.smart.script.trace.CoevalR.traced
import com.tacplatform.transaction.transfer.TransferTransaction
import monix.eval.Coeval
import shapeless._

object TacEnvironment {
  type In = Transaction :+: Order :+: PseudoTx :+: CNil
}

class TacEnvironment(
    nByte: Byte,
  ...
  import com.tacplatform.lang.v1.traits.Environment._
  ...
  def toAddress(recipient: Recipient): Option[com.tacplatform.account.Address] = {
    recipient match {
      case Address(bytes) =>
        com.tacplatform.account.Address
          .fromBytes(bytes.arr)
          .toOption
      case Alias(name) =>
        com.tacplatform.account.Alias
          .create(name)
  ...
  override def resolveAlias(name: String): Either[String, Recipient.Address] =
    ...
    blockchain
      .resolveAlias(com.tacplatform.account.Alias.create(name).explicitGet())
  ...
  override def accountBalanceOf(addressOrAlias: Recipient, maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        ...
        case Alias(name)    => com.tacplatform.account.Alias.create(name)
      }
  ...
  override def accountTacBalanceOf(addressOrAlias: Recipient): Either[String, Environment.BalanceDetails] = {
    (for {
      ...
      address <- blockchain.resolveAlias(aoa)
      portfolio = currentBlockchain().tacPortfolio(address)
...
object DAppEnvironment {
  ...
  final case class DAppInvocation(dAppAddress: com.tacplatform.account.Address, call: FUNCTION_CALL, payments: Seq[InvokeScriptTransaction.Payment])
}
...
class DAppEnvironment(
    ...
    currentDApp: com.tacplatform.account.Address,
    currentDAppPk: com.tacplatform.account.PublicKey,
    calledAddresses: Set[com.tacplatform.account.Address],
...
) extends TacEnvironment(nByte, in, h, blockchain, tthis, ds, tx.map(_.id()).getOrElse(ByteStr.empty)) {
  ...
  override def callScript(
    ...
    val invocation = InvokeScriptResult.Invocation(
      ...
      payments.map(p => InvokeScriptResult.AttachedPayment(p._1.fold(Asset.Tac: Asset)(a => IssuedAsset(ByteStr(a))), p._2)),
        ...
        account.Address
          ...
          .map(
            InvokeScript(
              ...
              payments.map(p => Payment(p._2, p._1.fold(Tac: Asset)(a => IssuedAsset(ByteStr(a))))),
              tx
            )
          )
      )
