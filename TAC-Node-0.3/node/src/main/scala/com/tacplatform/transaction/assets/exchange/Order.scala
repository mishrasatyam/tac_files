package com.tacplatform.transaction.assets.exchange
...
import com.tacplatform.transaction.assets.exchange.Validation.booleanOperators
import com.tacplatform.transaction.serialization.impl.OrderSerializer
...
case class Order(
  ...
  def isValid(atTime: Long): Validation = {
    ...
    (matcherFeeAssetId == Tac || version >= Order.V3) :| "matcherFeeAssetId should be tac"
  }
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr = if (version == 3) s" matcherFeeAssetId=${matcherFeeAssetId.fold("Tac")(_.toString)}," else ""
...
object Order {
  ...
  implicit lazy val jsonFormat: Format[Order] = com.tacplatform.transaction.assets.exchange.OrderJson.orderFormat
  ...
  def selfSigned(
      ...
      expiration: TxTimestamp,
      matcherFee: TxAmount,
      matcherFeeAssetId: Asset = Asset.Tac
  ): Order =
