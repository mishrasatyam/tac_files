package com.tacplatform.transaction.validation
...
import com.tacplatform.transaction.{Asset, TxValidationError, TxVersion, VersionedTransaction}
...
object TxConstraints {
  ...
  def asset[A <: Asset](asset: A): ValidatedV[A] = {
    asset.fold(Validated.validNel[ValidationError, A](asset)) { ia =>
      Validated
        .condNel(
          ia.id.arr.length == com.tacplatform.crypto.DigestLength,
          asset,
