...
import com.tacplatform.transaction.{Asset, Proofs, TxVersion}
...
object OrderJson {
  ...
  private val assetReads: Reads[Asset] = {
    case JsNull | JsString("") => JsSuccess(Tac)
    case JsString(s) =>
  ...
  implicit val assetPairReads: Reads[AssetPair] = {
    val r = (JsPath \ "amountAsset").readWithDefault[Asset](Tac)(assetReads) and
      (JsPath \ "priceAsset").readWithDefault[Asset](Tac)(assetReads)
    r(AssetPair(_, _))
  }
