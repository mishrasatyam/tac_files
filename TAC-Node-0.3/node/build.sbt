...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"tac-all-${version.value}.jar",
...
// Logback creates a "tac.directory_UNDEFINED" without this option.
scriptClasspath += "*"

bashScriptExtraDefines ++= Seq(
  s"""addJava "-Dtac.defaults.blockchain.type=${network.value}"""",
  s"""addJava "-Dtac.defaults.directory=/var/lib/${(Universal / normalizedName).value}"""",
  s"""addJava "-Dtac.defaults.config.directory=/etc/${(Universal / normalizedName).value}"""",
...
inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"tac-sample.conf" -> "doc/tac.conf.sample"),
    mappings := {
...
inConfig(Linux)(
  Seq(
    maintainer := "tacplatform.com",
    packageSummary := "Tac node",
    packageDescription := "Tac node"
  )
)
...
V.scalaPackage := "com.tacplatform"

// Hack for https://youtrack.jetbrains.com/issue/SCL-15210

moduleName := s"tac${network.value.packageSuffix}" // tac-*.jar instead of node-*.jar
executableScriptName := moduleName.value             // bin/tac instead of bin/node
